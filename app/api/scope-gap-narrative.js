...
${(gapReport.missingFromProposal || []).map(m =>
...
${(gapReport.exclusionConflicts || []).map(c =>
...
${(gapReport.quantityMismatches || []).map(q =>
...
Matched Divisions: ${(gapReport.matched || []).map(m => `Div ${m.division}`).join(', ') || 'None'}`;
    ...
    if (gapReport.coverageScore < 60 || gapReport.totalExposure > 50000) {
      riskLevel = 'high';
    } else if (gapReport.coverageScore < 80 || gapReport.exclusionConflicts?.length > 2 || gapReport.totalExposure > 10000) {
      riskLevel = 'medium';
    }
