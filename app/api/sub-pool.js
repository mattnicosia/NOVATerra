...
export default async function handler(req, res) {
  ...
  try {
    let query = supabaseAdmin.from("sub_pool").select("*").order("last_activity", { ascending: false }).limit(limit);
    ...
    const scored = (data || []).map(s => {
      ...
      return {
        ...s,
        _reputationScore:
          Math.round((proposals * 0.3 + wins * 0.4 + coverage * 0.003 - responseHrs * 0.001) * 100) / 100,
