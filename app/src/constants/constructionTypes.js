...
export const WORK_TYPES = [
  { key: "new-construction",     label: "New Construction",     multiplier: 1.00 },
  ...
  { key: "interior-fit-out",     label: "Interior Fit-Out",     multiplier: 0.60 },
  { key: "addition",             label: "Addition",             multiplier: 1.10 },
  { key: "adaptive-reuse",       label: "Adaptive Reuse",       multiplier: 1.20 },
...
export const BUILDING_TYPE_MAP = Object.fromEntries(
  BUILDING_TYPES.map(b => [b.key, b.label])
);

/** Map: workType key → { key, label, multiplier } */
export const WORK_TYPE_MAP = Object.fromEntries(
  WORK_TYPES.map(w => [w.key, w])
);

/** Map: outcome key → { key, label, color } */
export const OUTCOME_MAP = Object.fromEntries(
  OUTCOME_STATUSES.map(o => [o.key, o])
);

/** Get the ROM multiplier for a work type key. Falls back to 1.0 */
export const getWorkTypeMultiplier = (key) =>
  WORK_TYPE_MAP[key]?.multiplier ?? 1.0;

/** Get display label for a building type key */
export const getBuildingTypeLabel = (key) =>
  BUILDING_TYPE_MAP[key] || key || "Unclassified";

/** Get display label for a work type key */
export const getWorkTypeLabel = (key) =>
  WORK_TYPE_MAP[key]?.label || key || "";

/** Map: structural system key → display label */
export const STRUCTURAL_SYSTEM_MAP = Object.fromEntries(
  STRUCTURAL_SYSTEMS.map(s => [s.key, s.label])
);

/** Map: delivery method key → display label */
export const DELIVERY_METHOD_MAP = Object.fromEntries(
  DELIVERY_METHODS.map(d => [d.key, d.label])
);

/** Get outcome display info */
export const getOutcomeInfo = (key) =>
  OUTCOME_MAP[key] || { key: "pending", label: "Pending", color: "blue" };

/** Get display label for a structural system key */
export const getStructuralSystemLabel = (key) =>
  STRUCTURAL_SYSTEM_MAP[key] || key || "";

/** Get display label for a delivery method key */
export const getDeliveryMethodLabel = (key) =>
  DELIVERY_METHOD_MAP[key] || key || "";
