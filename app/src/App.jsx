...
function EstimateLoader({ children }) {
  ...
  if (loading || !persistenceLoaded || (!activeId && id)) {
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#8E8E93" }}>
        Loading estimate...
      </div>
    );
  }
  ...
  return (
    <div className="app-shell" style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden", background: C.bgGradient || C.bg, position: "relative" }}>
      ...
      <div className="app-main" style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
        <NovaHeader />
        <ProjectTabBar />
        <div className="app-viewport" style={{ flex: 1, position: "relative", overflow: isDashboard ? "hidden" : "auto", scrollBehavior: "smooth" }}>
          <PageTransition>
            ...
            <Routes>
              ...
              <Route path="/estimate/:id/documents" element={<EstimateLoader><PlanRoomPage /></EstimateLoader>} />
              ...
              <Route path="/estimate/:id/bids" element={<EstimateLoader><BidPackagesPage /></EstimateLoader>} />
      ...
      <Toast />
      <Suspense fallback={null}><AIChatPanel /></Suspense>
...
export default function App() {
  ...
  const [tourComplete, setTourComplete] = useState(
    () => localStorage.getItem('nova_tour_complete') === 'true'
  );
  // Progressive setup: first sign-in, runs after guided tour
  const [setupComplete, setSetupComplete] = useState(
    () => localStorage.getItem('nova_setup_complete') === 'true'
  );
