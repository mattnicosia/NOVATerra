...
export default function CostHistoryEntryForm({ onClose, onSave, initial, mode = "manual" }) {
  ...
  const remaining = total > 0 && computedTotal > 0 ? total - computedTotal : (total > 0 && divSum > 0 ? total - divSum : 0);
    ...
    <Modal onClose={onClose} width={720}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, marginBottom: 10, maxHeight: 220, overflowY: "auto" }}>
        {ROM_DIVISIONS.map(d => (
          <div key={d.code} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 6px", borderRadius: 4, background: form.divisions[d.code] ? `${C.accent}08` : "transparent" }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.textDim, fontFamily: "'DM Sans',sans-serif", width: 18 }}>{d.code}</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 8 }}>
        {MARKUP_CATEGORIES.filter(cat => cat.key !== "custom").map(cat => {
          ...
          return (
            <div key={cat.key} style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ fontSize: 8, color: catColor, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, minWidth: 60, opacity: 0.7 }}>
                {cat.label}
              </span>
              {catItems.map(p => (
                <button key={p.key} onClick={() => addMarkup(p.key)} disabled={usedKeys.has(p.key)}
                  style={{
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <span style={{ fontSize: 8, color: C.green, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, minWidth: 60, opacity: 0.7 }}>
            Custom
          </span>
          <button onClick={() => addMarkup("custom")}
            style={{
              background: `${C.green}10`, border: `1px solid ${C.green}30`,
              color: C.green, fontSize: 9, fontWeight: 600, padding: "2px 7px",
          ...
          {markups.map(m => {
            ...
            return (
              <div key={m.id} style={{
                ...
                borderRadius: 4, background: `${mColor}06`, border: `1px solid ${mColor}12`,
              }}>
                {/* Category color dot */}
                <div style={{ width: 6, height: 6, borderRadius: 3, flexShrink: 0, background: mColor }} />
                {/* Label */}
                {m.key === "custom" ? (
                  <input value={m.label} onChange={e => updateMarkup(m.id, "label", e.target.value)}
                    placeholder="Custom item name..."
                    style={inp(C, { flex: 1, padding: "3px 6px", fontSize: 10, minWidth: 120 })} />
                ) : (
                  <span style={{ flex: 1, fontSize: 10, fontWeight: 600, color: C.text, minWidth: 120 }}>{m.label}</span>
                )}

                {/* $/% toggle */}
                <div style={{ display: "flex", borderRadius: 3, overflow: "hidden", border: `1px solid ${C.border}` }}>
                  <button onClick={() => updateMarkup(m.id, "type", "dollar")}
                    style={{
                      background: m.type === "dollar" ? C.accent : "transparent",
                      color: m.type === "dollar" ? "#fff" : C.textDim,
                      border: "none", fontSize: 9, fontWeight: 700, padding: "2px 6px", cursor: "pointer",
                    }}>$</button>
                  <button onClick={() => updateMarkup(m.id, "type", "percent")}
                    style={{
                ...
                {/* Input value */}
                <input type="number" value={m.inputValue}
                ...
                {/* Remove */}
                <button onClick={() => removeMarkup(m.id)}
        ...
        <div style={{ padding: "8px 10px", borderRadius: 6, background: C.bg2, marginBottom: 10, fontSize: 10 }}>
          ...
          {markupSum > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ color: C.textDim }}>+ Markups ({markups.filter(m => parseFloat(m.inputValue) > 0).length} items)</span>
          ...
          {total > 0 && computedTotal > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{
                ...
                {fmtCost(total)}
                {Math.abs(total - computedTotal) < 2 ? " ✓" : ` (${total - computedTotal > 0 ? "+" : ""}${fmtCost(total - computedTotal)} gap)`}
              </span>
            </div>
          )}
