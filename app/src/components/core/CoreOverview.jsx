...
const KPI_ICONS = {
  ...
  costItems: "M12 2C6.48 2 2 4.02 2 6.5S6.48 11 12 11s10-2.02 10-4.5S17.52 2 12 2z M2 6.5v5c0 2.48 4.48 4.5 10 4.5s10-2.02 10-4.5v-5",
  assemblies: "M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z M3.27 6.96L12 12.01l8.73-5.05 M12 22.08V12",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{
          ...
          <svg width={14} height={14} viewBox="0 0 24 24" fill="none"
            stroke={color || accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            <path d={icon} />
          </svg>
        </div>
...
export default function CoreOverview() {
  ...
  const stats = useMemo(() => {
    ...
    const filteredProposals = activeCompanyId === "__all__" ? proposals
      : !activeCompanyId ? proposals.filter(p => !p.companyProfileId)
    ...
    userElements.forEach(e => { if (e.trade) trades.add(e.trade); });
    filteredProposals.forEach(p => {
      const divs = p.divisions;
      if (Array.isArray(divs)) {
        divs.forEach(d => { if (d.code) trades.add(d.code); });
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              background: `${a.color}0A`, border: `1px solid ${a.color}20`,
            ...
            onMouseLeave={e => { e.currentTarget.style.background = `${a.color}0A`; e.currentTarget.style.borderColor = `${a.color}20`; }}
          >
      ...
      {stats.proposalCount === 0 && stats.elementCount === 0 && (
        <div style={{
          ...
          <p style={{ fontSize: 12, color: C.textMuted, margin: "0 0 16px", maxWidth: 380, marginInline: "auto" }}>
            Upload your first proposal or import cost data to begin training NOVA's intelligence. Every data point makes your estimates more accurate.
          </p>
          <button
