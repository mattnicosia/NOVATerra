...
export default function TakeoffNOVAPanel({
    ...
    if (tkActiveTakeoffId) {
      ...
      if (pred.type === "count" || pred.type === "wall-tag") {
        addMeasurement(tkActiveTakeoffId, {
          ...
          sheetId: selectedDrawingId, color, predicted: true, tag: tkPredictions.tag,
        });
        ...
        addMeasurement(tkActiveTakeoffId, {
          ...
          sheetId: selectedDrawingId, color, predicted: true, tag: pred.tag || tkPredictions.tag,
        });
      }
    }
  };
  ...
  const handleNovaChat = async text => {
    ...
    try {
      ...
      if (typeof resp === "string") {
        ...
        // In estimate context → intercept tool calls as proposals for approval
        if (isEstimate && toolCalls.length > 0) {
          const newProposals = [];
          ...
          if (newProposals.length > 0) {
            ...
            // No valid previews — just show text
            setNovaChatMessages([...updated, { role: "assistant", text: textParts.join("\n") || "No changes to propose." }]);
          }
        } else {
          // Takeoff context or no tool calls → execute immediately (existing behavior)
          const toolResults = toolCalls.map(tc => {
            try { return { tool_use_id: tc.id, ...executeNovaTool(tc.name, tc.input) }; }
            catch (err) { return { tool_use_id: tc.id, success: false, message: err.message }; }
          });
          setNovaChatMessages([...updated, { role: "assistant", text: textParts.join("\n") || "", actions: toolResults }]);
        }
      }
  ...
  const takeoffPresets = [
    {
      key: "detect", label: "Detect Elements", icon: I.eye, color: C.accent,
    ...
    {
      key: "schedules", label: "Find Schedules", icon: I.schedule, color: "#10B981",
    ...
    {
      key: "scanAll", label: "Scan All Pages", icon: I.plans, color: "#3B82F6",
      loading: crossSheetScan?.scanning,
      badge: crossSheetScan?.results?.length > 0 ? `${crossSheetScan.results.length}` : null,
      action: () => {
        if (!tkPredictions?.tag) { showToast("Run a detection first"); return; }
    ...
    {
      key: "categorize", label: "Categorize Items", icon: I.hash, color: "#F59E0B",
      loading: false, badge: null,
      action: () => handleNovaChat("Review all estimate items and categorize any unallocated scope items into appropriate CSI divisions. Flag items that need attention."),
    },
  ];

  const estimatePresets = [
    {
      key: "suggest", label: "Suggest Items", icon: I.plusCircle, color: C.accent,
      loading: false, badge: null,
      action: () => handleNovaChat("Based on the project type, scanned documents, extracted schedules, and existing estimate items, suggest line items that are likely missing. For each suggestion include a description, CSI code, suggested quantity, and unit. Focus on items that typically accompany what's already in the estimate. Use the add_line_items tool to propose them."),
    },
    {
      key: "specs", label: "Assign Specs", icon: I.file, color: "#10B981",
      loading: false, badge: null,
      action: () => handleNovaChat("Review all estimate items and match them to extracted specification sections. For each item, assign the most relevant spec section using the update_line_items tool. Note any spec requirements that affect pricing or scope."),
    },
    {
      key: "price", label: "Price Items", icon: I.dollar, color: "#3B82F6",
      loading: false, badge: null,
      action: () => handleNovaChat("Review all estimate items that have $0 or missing pricing. For each unpriced item, suggest material, labor, equipment, and/or subcontractor unit costs based on the item description, quantity, unit, and project context. Use the update_line_items tool to propose prices. Reference your source for each price."),
    },
    {
      key: "categorize", label: "Categorize Items", icon: I.hash, color: "#F59E0B",
      loading: false, badge: null,
      action: () => handleNovaChat("Review all estimate items and categorize any unallocated scope items into appropriate CSI divisions. Use the update_line_items tool to assign proper codes and divisions. Flag items that need attention."),
    },
    {
      key: "hideZero", label: "Hide $0 Items", icon: I.eyeOff, color: C.textDim,
      loading: false, badge: null, isToggle: true,
      action: () => {
    ...
    {
      key: "organize", label: "Organize", icon: I.layers, color: "#8B5CF6",
      loading: false, badge: null,
      action: () => handleNovaChat("Review and organize the estimate: clean up descriptions to be consistent and professional, ensure CSI codes are correct and complete, suggest a logical ordering within each division. Use the update_line_items tool to apply changes."),
    },
  ];
    ...
    >
          ...
          {pendingProposals.length > 0 && (
            <span
              style={{
                ...
                background: C.accent, borderRadius: 6, padding: "1px 6px",
                lineHeight: 1.4,
              }}
            >
              {pendingProposals.reduce((s, p) => s + p.previews.length - p.acceptedIds.size - p.rejectedIds.size, 0)} pending
            </span>
          )}
        </div>
          ...
          <div style={{ flexShrink: 0 }}>
            <div
              style={{
                ...
                display: "flex", alignItems: "center", gap: 8, background: `${novaPredColor}04`,
              }}
            >
            ...
            {novaPending.length > 0 && (
              <div style={{ padding: "6px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 6 }}>
                <button onClick={novaAcceptAll} style={bt(C, { flex: 1, background: C.green, color: "#fff", padding: "6px 0", fontSize: 10, fontWeight: 700, borderRadius: 5, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 })}>
                  <Ic d={I.check} size={10} color="#fff" sw={2.5} /> Accept All ({novaPending.length})
                </button>
                <button onClick={novaRejectAll} style={bt(C, { flex: 1, background: "transparent", border: `1px solid ${C.border}`, color: C.textDim, padding: "6px 0", fontSize: 10, fontWeight: 600, borderRadius: 5 })}>
                  Dismiss All
                </button>
              </div>
            )}
              ...
              {novaPreds.map(pred => {
                ...
                return (
                  <div key={pred.id} style={{
                    margin: "2px 0", padding: "6px 10px", borderRadius: 6,
                    ...
                    opacity: isRejected ? 0.45 : 1, transition: T.transition.fast,
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: isPending ? 5 : 0 }}>
                      <div style={{ width: 5, height: 5, borderRadius: "50%", flexShrink: 0, background: isAccepted ? C.green : isRejected ? C.red : novaPredColor }} />
                      <span style={{ fontSize: 10, fontWeight: 600, color: C.text, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: T.font.sans }}>
                        {pred.tag || pred.type || "Prediction"}
                      </span>
                      <span style={{ fontSize: 9, fontWeight: 700, color: confColor, fontFamily: T.font.sans }}>{predConf}%</span>
                    ...
                    {isPending && (
                      <div style={{ display: "flex", gap: 4 }}>
                        <button onClick={() => novaAcceptOne(pred)} style={bt(C, { flex: 1, padding: "3px 0", fontSize: 9, fontWeight: 600, borderRadius: 4, background: `${C.green}12`, color: C.green, border: `1px solid ${C.green}20`, display: "flex", alignItems: "center", justifyContent: "center", gap: 3 })}>
                          <Ic d={I.check} size={8} color={C.green} sw={2.5} /> Accept
                        </button>
                        <button onClick={() => { rejectPrediction(pred.id); recordPredictionFeedback(tkPredictions?.tag, tkPredictions?.strategy, false); }} style={bt(C, { flex: 1, padding: "3px 0", fontSize: 9, fontWeight: 600, borderRadius: 4, background: "transparent", color: C.textDim, border: `1px solid ${C.border}` })}>
                          Dismiss
                        </button>
                      </div>
                    )}
            ...
            {tkPredRefining && (
              <div style={{ padding: "6px 14px", borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, background: `${C.orange}04`, fontSize: 9, color: C.orange, fontWeight: 600 }}>
        ...
        <div style={{ flex: 1, padding: "0 10px" }}>
          {novaChatMessages.length === 0 && !novaChatLoading && novaPreds.length === 0 && proposals.length === 0 && (
            <div style={{ padding: "20px 4px 8px", textAlign: "center" }}>
              <div style={{ fontSize: 10, color: C.textDim, fontFamily: T.font.sans, fontWeight: 500, lineHeight: 1.5 }}>
          ...
          {novaChatLoading && (
            <div style={{ display: "flex", gap: 4, padding: "8px 0", alignItems: "center" }}>
              {[0, 1, 2].map(n => (
                <div key={n} style={{ width: 4, height: 4, borderRadius: 2, background: C.accent, opacity: 0.4, animation: `novaPulse 1.2s ${n * 0.2}s infinite` }} />
              ))}
              <span style={{ fontSize: 9, color: C.textMuted, marginLeft: 3, fontFamily: T.font.sans }}>Thinking...</span>
            </div>
          )}
        </div>
        ...
        {proposals.length > 0 && (
          <div style={{ padding: "6px 8px 2px", flexShrink: 0 }}>
            {proposals.map(proposal => {
              ...
              const typeLabel =
                proposal.toolName === "add_line_items" ? "Add Items" :
                proposal.toolName === "update_line_items" ? "Update Items" :
                proposal.toolName === "remove_line_items" ? "Remove Items" : "Changes";
              const typeColor =
                proposal.toolName === "add_line_items" ? (C.green || "#10B981") :
                proposal.toolName === "update_line_items" ? (C.blue || "#3B82F6") :
                proposal.toolName === "remove_line_items" ? (C.red || "#EF4444") : C.accent;
              ...
              return (
                ...
                >
                  ...
                  >
                    <Ic
                      d={proposal.toolName === "add_line_items" ? I.plusCircle :
                         proposal.toolName === "update_line_items" ? I.edit :
                         I.trash}
                  ...
                  {!isResolved && remaining > 1 && (
                    <div style={{ padding: "5px 10px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 5 }}>
                      <button
                        onClick={() => acceptAllInProposal(proposal.id)}
                        style={bt(C, {
                          flex: 1, padding: "4px 0", fontSize: 9, fontWeight: 700,
                          borderRadius: 4, background: C.green, color: "#fff",
                      ...
                      <button
                        ...
                        style={bt(C, {
                          ...
                          borderRadius: 4, background: "transparent", color: C.textDim,
                    ...
                    {proposal.previews.map((preview, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          style={{
                            margin: "2px 0", padding: "6px 8px", borderRadius: 6,
                            background: isAccepted ? `${C.green}06` : isRejected ? `${C.red}04` : C.bg,
                            border: `1px solid ${isAccepted ? C.green + "18" : isRejected ? C.red + "12" : C.border}`,
                        ...
                        >
                          {/* Preview content */}
                          {preview.type === "add" && (
                            <ProposalAddCard preview={preview} C={C} T={T} />
                          )}
                          {preview.type === "update" && (
                            <ProposalUpdateCard preview={preview} C={C} T={T} />
                          )}
                          {preview.type === "remove" && (
                            <ProposalRemoveCard preview={preview} C={C} T={T} />
                          )}

                          {/* Status badge for handled items */}
                          {isAccepted && (
                            <div style={{ marginTop: 4, fontSize: 8, fontWeight: 700, color: C.green, display: "flex", alignItems: "center", gap: 3 }}>
                              <Ic d={I.check} size={8} color={C.green} sw={2.5} /> Applied
                            </div>
                          )}
                          {isRejected && (
                            <div style={{ marginTop: 4, fontSize: 8, fontWeight: 700, color: C.textDim }}>Dismissed</div>
                          )}
                            ...
                            <div style={{ display: "flex", gap: 4, marginTop: 5 }}>
                              <button
                                ...
                                style={bt(C, {
                                  ...
                                  borderRadius: 4, background: `${C.green}12`, color: C.green,
                  ...
                  {isResolved && (
                    <div
                      ...
                      style={{
                        padding: "4px 10px", textAlign: "center", fontSize: 8,
                        color: C.textDim, cursor: "pointer", borderTop: `1px solid ${C.border}`,
                      }}
                    >
                      Clear
                    </div>
      ...
      <div style={{ flexShrink: 0, borderTop: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, padding: "8px 10px 6px" }}>
          {presets.map(p => (
            <button
              ...
              disabled={p.loading}
              style={{
                padding: "7px 6px", background: C.bg, border: `1px solid ${C.border}`,
                borderRadius: 6, cursor: p.loading ? "wait" : "pointer",
                textAlign: "center", transition: "all 0.15s", position: "relative",
              }}
              onMouseEnter={e => { if (!p.loading) e.currentTarget.style.borderColor = p.color + "40"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; }}
            >
              {p.loading ? (
                <div style={{ width: 14, height: 14, margin: "0 auto 4px", border: `2px solid ${p.color}25`, borderTop: `2px solid ${p.color}`, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
              ) : (
              ...
              {p.badge && (
                <span style={{ position: "absolute", top: 3, right: 3, fontSize: 7, fontWeight: 800, color: "#fff", background: p.color, borderRadius: 6, padding: "1px 4px", lineHeight: 1.3 }}>
                  {p.badge}
                </span>
              )}
            </button>
        ...
        <div style={{ padding: "4px 10px 10px" }}>
          <div style={{ display: "flex", gap: 6, alignItems: "flex-end", background: C.bg, borderRadius: 7, border: `1px solid ${C.border}`, padding: "5px 8px" }}>
            <textarea
              ref={inputRef}
              value={novaChatInput}
              ...
              rows={1}
              style={{ flex: 1, border: "none", outline: "none", resize: "none", background: "transparent", color: C.text, fontSize: 11, fontFamily: T.font.sans, lineHeight: 1.4, maxHeight: 72, padding: 0 }}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 2 }}>
        {preview.code && (
          <span style={{ fontSize: 8, fontWeight: 700, color: C.accent, fontFamily: T.font.sans, fontFeatureSettings: "'tnum'" }}>
            {preview.code}
          </span>
        )}
        <span style={{ fontSize: 10, fontWeight: 600, color: C.text, fontFamily: T.font.sans, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {preview.description}
        </span>
      </div>
      <div style={{ display: "flex", gap: 8, fontSize: 8, color: C.textDim, fontFamily: T.font.sans, fontFeatureSettings: "'tnum'" }}>
        <span>{preview.quantity} {preview.unit}</span>
        {preview.division && <span style={{ color: C.textMuted }}>{preview.division}</span>}
        {totalUnit > 0 && (
          <span style={{ color: C.green, fontWeight: 600 }}>
            {formatCurrency(totalUnit)}/unit
          </span>
        )}
      </div>
  ...
  return (
    <div>
      ...
      {changeEntries.map(([field, { before, after }]) => {
        ...
        const fieldLabels = {
          material: "Material", labor: "Labor", equipment: "Equip",
          subcontractor: "Sub", quantity: "Qty", unit: "Unit",
          code: "Code", description: "Desc", division: "Division",
          trade: "Trade", specSection: "Spec", notes: "Notes",
        };
        return (
          <div key={field} style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2, fontSize: 8, fontFamily: T.font.sans }}>
            <span style={{ color: C.textDim, width: 46, flexShrink: 0, fontWeight: 600 }}>
              {fieldLabels[field] || field}
            </span>
            <span style={{ color: C.red + "cc", textDecoration: "line-through", fontFeatureSettings: isCost ? "'tnum'" : undefined }}>
...
function ProposalRemoveCard({ preview, C, T }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
      ...
      {preview.code && (
        <span style={{ fontSize: 8, color: C.textDim, fontFamily: T.font.sans }}>{preview.code}</span>
      )}
    </div>
  );
}
