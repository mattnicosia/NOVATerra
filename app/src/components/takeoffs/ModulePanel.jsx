    ...
    return (
      ...
      >
        ...
        {toId && hasQty && !isExcluded ? (
          <div style={{ width: 10, height: 10, borderRadius: 2, flexShrink: 0, cursor: "pointer", position: "relative",
            background: takeoffs.find(t => t.id === toId)?.color || C.accent }}
            onClick={e => { e.stopPropagation(); e.currentTarget.querySelector('input')?.click(); }}>
            <input type="color" value={takeoffs.find(t => t.id === toId)?.color || "#2563eb"}
              onChange={e => { e.stopPropagation(); updateTakeoff(toId, "color", e.target.value); }}
    ...
    return (
      <div style={{ margin: "4px 8px" }}>
        ...
        {templates && templates.length > 0 && (
          <div style={{ padding: "5px 8px", background: `linear-gradient(135deg, ${C.bg2}, ${C.bg1})`, borderRadius: "5px 5px 0 0", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 8, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>Quick Start</span>
            {templates.map((tmpl, ti) => (
              <button key={ti} onClick={() => applyTemplate(tmpl)}
                style={{ padding: "3px 8px", fontSize: 9, fontWeight: 600, border: `1px solid ${C.accent}30`, background: C.bg1, color: C.text, borderRadius: 10, cursor: "pointer", transition: "all 0.15s", whiteSpace: "nowrap" }}
                onMouseEnter={e => { e.currentTarget.style.background = C.accent + '15'; e.currentTarget.style.borderColor = C.accent; }}
                onMouseLeave={e => { e.currentTarget.style.background = C.bg1; e.currentTarget.style.borderColor = C.accent + '30'; }}
              >{tmpl.label}</button>
            ))}
          </div>
        )}
        {materialSpec && (
          <div style={{ padding: "6px 8px", background: C.bg2, borderRadius: templates?.length ? 0 : "5px 5px 0 0", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 4 }}>
        ...
        {(() => {
          ...
          const getGroup = (spec) => {
            ...
            if (id.startsWith("Sheath") || id === "WrbType" || id === "MSSheathing" || id.includes("Sheathing")) return "Sheathing";
            if (id.startsWith("Insul") || id.includes("Insul") || id === "WrbType") return "Insulation & WRB";
            if (id.startsWith("Roof") && (id.includes("Finish") || id.includes("Underlayment") || id.includes("Flashing") || id.includes("RidgeVent"))) return "Roofing Finish";
          ...
          return groups.map((g, gi) => {
            ...
            return (
              <div key={g}>
                <div onClick={() => toggleSpecGroup(groupKey)}
                  style={{ padding: "4px 8px", background: C.bg2, borderBottom: isCollapsed && !isLast ? `1px solid ${C.border}` : "none", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", userSelect: "none" }}>
                  ...
                  <span style={{ fontSize: 8, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>{g}</span>
                  {customized > 0 && <span style={{ fontSize: 7, color: C.accent, fontWeight: 600 }}>({customized} customized)</span>}
                </div>
                {!isCollapsed && (
                  <div style={{ padding: "6px 8px", background: C.bg2, borderRadius: isLast ? "0 0 5px 5px" : 0, display: "flex", gap: 4, flexWrap: "wrap", borderBottom: !isLast ? `1px solid ${C.border}20` : "none" }}>
