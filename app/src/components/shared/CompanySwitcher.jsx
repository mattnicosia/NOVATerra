...
export default function CompanySwitcher() {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
      {/* All */}
      <button onClick={() => set("__all__")} title="All Companies" style={cardStyle(activeCompanyId === "__all__")}>
        <Ic d={I.layers} size={16} color={activeCompanyId === "__all__" ? C.accent : C.textDim} />
      </button>

      {/* Primary profile */}
      <button onClick={() => set("")} title={companyInfo?.name || "Primary"} style={cardStyle(activeCompanyId === "")}>
        <LogoPill
