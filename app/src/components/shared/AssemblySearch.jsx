...
export default function AssemblySearch({ onInsertAssembly, onInsertItem, placeholder }) {
  ...
  const localResults = useMemo(() => {
    ...
    const projAsmResults = projectAssemblies
      .filter(
        a =>
  ...
  const hasResults =
    localResults.projectAssemblies.length > 0 || localResults.assemblies.length > 0 || mergedItems.length > 0;
              ...
              {localResults.projectAssemblies.map(asm => {
                ...
                return (
                  <div key={asm.id}>
                    <div
                      className="nav-item"
                      onClick={() => setPreviewAsm(isPreviewing ? null : asm)}
                      style={{
                        padding: "8px 10px",
                        display: "flex",
                        alignItems: "center",
                        ...
                        cursor: "pointer",
                        background: isPreviewing ? C.accentBg : "transparent",
                      }}
                    >
                      ...
                      <span
                        style={{ fontFamily: T.font.sans, fontSize: 9, color: C.accent, fontWeight: 600 }}
                      >
                        {fmt2(totalPer)}/ea
                      </span>
                    </div>
                    {isPreviewing && (
                      <div style={{ padding: "8px 12px", background: C.bg2, borderBottom: `1px solid ${C.border}` }}>
                        {asm.elements.map((el, i) => (
                          ...
                          >
                            <span>{el.desc}</span>
                            <span style={{ fontFamily: T.font.sans, fontSize: 9, color: C.accent }}>
                              {fmt2((nn(el.m) + nn(el.l) + nn(el.e)) * nn(el.factor))}
                            </span>
                          </div>
                        ))}
            ...
            <>
              ...
              {mergedItems.map(el => (
                ...
                >
                  ...
                  <span style={{ fontFamily: T.font.sans, fontSize: 9, color: C.accent, fontWeight: 600 }}>
