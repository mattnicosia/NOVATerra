...
export default function NewEstimateModal({ onCreated, onClose, companyProfileId }) {
  ...
  const handleCreate = async () => {
    ...
    if (!trimmed) { setError("Estimate number is required"); return; }
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ padding: T.space[5] }}>
        ...
        <label style={{ fontSize: 10, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>
          Estimate Number
        </label>
        <input
          autoFocus
          value={estNum}
          onChange={e => { setEstNum(e.target.value); setError(""); }}
          onKeyDown={e => e.key === "Enter" && handleCreate()}
          placeholder="e.g. EST-2026-001"
          style={inp(C, { width: "100%", marginTop: 4, marginBottom: error ? 4 : T.space[4], fontSize: 13 })}
        />
        {error && (
          <div style={{ fontSize: 11, color: C.red || "#f44", marginBottom: T.space[3] }}>
            {error}
          </div>
        )}
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onClose}
            style={bt(C, { padding: "8px 16px", fontSize: 12, background: "transparent", color: C.textMuted, border: `1px solid ${C.border}` })}>
            Cancel
          </button>
          <button onClick={handleCreate} disabled={creating}
            style={bt(C, { padding: "8px 16px", fontSize: 12, background: C.accent, color: "#fff", opacity: creating ? 0.6 : 1 })}>
