...
// Status → color
const STATUS_COLORS = {
  Bidding: '#A78BFA', Submitted: '#F59E0B', Won: '#34D399',
  Lost: '#F87171', 'On Hold': '#A78BFA', Draft: '#8E8E93',
};
...
export default function CommandPalette() {
  ...
  const results = useMemo(() => {
    ...
    if (!query && recentIds.length > 0) {
      ...
      if (recentEstimates.length > 0) {
        recentEstimates.forEach(e => {
          items.push({
            id: `recent-${e.id}`, label: e.name, sub: e.client || e.status || '',
            icon: I.estimate, category: 'Recent', estimateId: e.id,
            status: e.status,
          });
        });
      }
    ...
    filtered.forEach(e => {
      ...
      items.push({
        id: `est-${e.id}`, label: e.name, sub: e.client || e.status || '',
        icon: I.estimate, category: 'Estimates', estimateId: e.id,
        status: e.status,
      });
    });
  ...
  const execute = useCallback((item) => {
    ...
    if (item.estimateId) {
      ...
      navigate(`/estimate/${item.estimateId}/estimate`);
    } else if (item.path) {
      navigate(item.path);
