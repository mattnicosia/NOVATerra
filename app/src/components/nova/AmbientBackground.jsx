...
export default function AmbientBackground() {
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      if (now - lastFrame < 50) { animRef.current = requestAnimationFrame(draw); return; }
      lastFrame = now;
      tRef.current += 0.006;
      ...
      const prlxX = ((W / 2 - mx) / W) * 3;
      const prlxY = ((H / 2 - my) / H) * 3;
  ...
  return (
    <>
      ...
      {/* Ambient CSS blobs */}
      <div style={{
        position: 'fixed', width: 580, height: 580, top: -180, right: 60,
      ...
      <div style={{
        position: 'fixed', width: 420, height: 420, bottom: -100, left: 80,
        borderRadius: '50%', filter: 'blur(80px)', pointerEvents: 'none', zIndex: 0,
        background: 'radial-gradient(ellipse, rgba(40,60,120,0.014) 0%, transparent 70%)',
        animation: 'drift 28s ease-in-out infinite alternate-reverse',
      }} />
      <div style={{
        position: 'fixed', width: 320, height: 320, top: '50%', left: '50%',
