// NOVACORE fragment shader v11.5 — CELESTIAL BODY
// NOVA: a blue star in deep space — dramatic limb darkening, blazing center,
// visible convection cells, deep indigo edges dissolving into corona.
// CORE: amber fusion reactor — volcanic, turbulent, textured.
// v11.5: STELLAR REALISM — aggressive center-to-edge falloff, visible
// surface granulation, HDR center bloom, stronger color zone contrast.
...
export const novacoreFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // v11.5b: Solar granulation — visible convection cells at stellar scale.
    // Freq 3.5-6.0 creates 4-8 visible cells across the sphere face.
    // Amplitude ±20% — visible but not dominant (color flow is the hero).
    // Two layers at different scales for organic, non-repeating variation.
    float granSpeed = mix(0.05, 0.0, uMorph);  // NOVA: slow drift. CORE: off.
    float gran1 = snoise(vPosition * 3.5 + uTime * vec3(granSpeed, granSpeed * 0.7, granSpeed * 1.2));
    float gran2 = snoise(vPosition * 5.5 + uTime * vec3(granSpeed * 0.8, granSpeed * 1.1, granSpeed * 0.6) + vec3(4.1, 2.3, 7.0));
    float granulation = (gran1 * 0.12 + gran2 * 0.08) * (1.0 - uMorph);  // ±20% at NOVA
    // Granulation visible everywhere except dead center (blown out by HDR).
    // Edge fading removed — limb darkening naturally suppresses edge brightness,
    // so granulation at the edge adds subtle structure to the dark limb.
    float granMask = smoothstep(0.0, 0.15, 1.0 - centerFactor);
    granulation *= granMask;
    flowBright *= 1.0 + granulation;
    ...
    // This creates the "blazing center dissolving to dark limb" look of real stars.
    float limbDarkenRG = mix(
      smoothstep(0.0, 0.60, centerFactor),           // NOVA: dramatic S-curve falloff
      0.80 + 0.20 * centerFactor,                     // CORE: mild
      uMorph
    );
    float limbDarkenB = mix(
      0.05 + 0.95 * smoothstep(0.0, 0.50, centerFactor),  // NOVA: B keeps 5% floor (deep indigo)
      0.82 + 0.18 * centerFactor,                           // CORE: mild
      uMorph
    );
