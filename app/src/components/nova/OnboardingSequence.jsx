...
export default function OnboardingSequence({ onComplete, onTransitionStart }) {
  ...
  useEffect(() => {
    ...
    voice.preWarm([
      S.firstContact.line1.text,
      S.firstContact.line2.text,
      S.question.line.text,
  ...
  useEffect(() => {
    ...
    later(() => { setNovaState('idle'); sound.setDroneState('idle'); }, S.firstContact.thinkingFlashMs);
    later(() => {
      setLines([{ key: nextKey(), lineId: 'fc-1', text: S.firstContact.line1.text, style: textStyle, voiceOpts: VOICE_PRESETS[S.firstContact.line1.voice] }]);
  ...
  const handleFirstContactLine1Done = useCallback(() => {
    later(() => {
      setLines(prev => [
        ...
        { key: nextKey(), lineId: 'fc-2', text: S.firstContact.line2.text, style: textStyle, voiceOpts: VOICE_PRESETS[S.firstContact.line2.voice] },
      ]);
  ...
  useEffect(() => {
    ...
    later(() => {
      ...
      setTextFade(1);
      later(() => {
        setLines([{ key: nextKey(), lineId: 'q-1', text: S.question.line.text, style: textStyle, voiceOpts: VOICE_PRESETS[S.question.line.voice] }]);
  ...
  useEffect(() => {
    ...
    setTextFade(0);
    later(() => { setLines([]); setTextFade(1); }, S.recognition.textFadeMs);
    ...
    later(() => {
      ...
      later(() => {
        setLines([{ key: nextKey(), lineId: 'r-greet', text: S.recognition.greetLine.text(firstName), style: textStyle, voiceOpts: VOICE_PRESETS[S.recognition.greetLine.voice] }]);
  ...
  const handleNameEchoDone = useCallback(() => {
    ...
    later(() => {
      setLines(prev => [
        ...prev.map(l => ({ ...l, done: true })),
        { key: nextKey(), lineId: 'r-role', text: S.recognition.roleLine.text, style: textStyle, voiceOpts: VOICE_PRESETS[S.recognition.roleLine.voice] },
      ]);
  ...
  useEffect(() => {
    ...
    later(() => {
      setLines([{ key: nextKey(), lineId: 'absorb-1', text: S.absorb.line.text, style: textStyle, voiceOpts: VOICE_PRESETS[S.absorb.line.voice] }]);
  ...
  useEffect(() => {
    ...
    setTextFade(0);
    later(() => { setLines([]); setTextFade(1); }, S.activation.textFadeMs);
