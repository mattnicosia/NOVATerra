...
export default function ImportConfirmModal({ rfp, onClose, onConfirm, loading }) {
  ...
  const fieldDefs = [
    ...
    { key: "jobType", label: "Job Type", type: "select", options: ["", ...(Array.isArray(masterData.jobTypes) ? masterData.jobTypes.map(j => typeof j === "string" ? j : j.name) : [])] },
    ...
    <Modal onClose={onClose} wide={!hasPreview} extraWide={hasPreview}>
      <div style={{ display: "flex", gap: T.space[5] }}>
        ...
        <div style={{ flex: hasPreview ? "0 0 380px" : 1, minWidth: 0 }}>
          ...
          {/* Editable fields */}
          <div style={{ display: "grid", gridTemplateColumns: hasPreview ? "1fr" : "1fr 1fr", gap: T.space[3], marginBottom: T.space[5] }}>
            {fieldDefs.map(fd => (
              <div key={fd.key} style={(!hasPreview && fd.key === "description") ? { gridColumn: "span 2" } : {}}>
                <label style={{
                  display: "block", fontSize: T.fontSize.xs, color: C.textDim,
                  fontWeight: T.fontWeight.semibold, textTransform: "uppercase",
          ...
          {attachments.length > 0 && (
            <div style={{
              padding: T.space[3], borderRadius: T.radius.sm,
              background: C.bg, border: `1px solid ${C.border}`, marginBottom: T.space[4],
            }}>
              <div style={{ fontSize: T.fontSize.xs, color: C.textDim, fontWeight: T.fontWeight.semibold, marginBottom: T.space[2] }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        fontSize: T.fontSize.xs, padding: "5px 8px", borderRadius: T.radius.sm,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: "1px solid transparent", transition: T.transition.fast,
                      }}
                    >
          ...
          {/* Actions */}
          <div style={{ display: "flex", justifyContent: "flex-end", gap: T.space[3], borderTop: `1px solid ${C.border}`, paddingTop: T.space[4] }}>
            <button style={bt(C, { padding: "8px 20px", background: "transparent", color: C.textMuted, border: `1px solid ${C.border}` })} onClick={onClose} disabled={loading}>
              Cancel
            </button>
            <button
