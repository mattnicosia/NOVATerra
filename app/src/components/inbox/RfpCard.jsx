...
export default function RfpCard({ rfp, isUnread, onView, onImport, onDismiss, onRetry }) {
  ...
  const accentColor = isUnread ? C.accent
    : isError ? "#ef4444"
    : isImported ? "#22c55e"
    : isPending ? "#f59e0b"
    : null;
  ...
  return (
    ...
    >
      <div style={{ display: "flex" }}>
        ...
        {accentColor && (
          <div style={{
            width: 4, flexShrink: 0, background: accentColor,
        ...
        <div style={{ flex: 1, padding: T.space[5] }}>
          ...
          {isError && rfp.parse_error && (
            <div style={{
              fontSize: T.fontSize.xs, color: "#ef4444", marginBottom: T.space[3],
              padding: "4px 8px", borderRadius: T.radius.sm, background: "#ef444410",
            }}>
          ...
          {pd.projectName && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: T.space[2], marginBottom: T.space[3] }}>
              ...
              {pd.jobType && (
                <span style={{
                  fontSize: T.fontSize.xs, padding: "2px 6px", borderRadius: T.radius.sm,
                  background: "#8b5cf618", color: "#8b5cf6", fontWeight: T.fontWeight.medium,
                }}>
                  {pd.jobType}
                </span>
              )}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{
              ...
              color: C.textDim, fontSize: T.fontSize.xs,
            }}>
              ...
              {pd.planLinks?.length > 0 && (
                <>
                  ...
                  <span>{pd.planLinks.length} plan link{pd.planLinks.length !== 1 ? "s" : ""}</span>
                </>
              )}
            </div>
            <div style={{ display: "flex", gap: T.space[2] }} onClick={e => e.stopPropagation()}>
              ...
              {isError && onRetry && (
                <>
                  <button style={bt(C, {
                    padding: "4px 12px", background: "#f59e0b", color: "#fff", fontSize: T.fontSize.xs,
