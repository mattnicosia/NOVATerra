...
export function BarChart({ data, height = 80, showLabels = true, showValues = false, animate = true, barColor, maxOverride }) {
  const C = useTheme();
...
export function RangeBar({ low, mid, high, marker, color, markerColor, markerLabel, height = 6 }) {
  ...
  return (
    <div style={{ position: "relative", height: height + 12 }}>
      {/* Track */}
      <div style={{ position: "absolute", left: 0, right: 0, top: 6, height, borderRadius: height / 2, background: `${C.bg2}` }} />
      {/* Range fill */}
      <div style={{
        position: "absolute", left: `${lowPct}%`, width: `${highPct - lowPct}%`, top: 6, height,
        borderRadius: height / 2, background: `linear-gradient(90deg, ${c}40, ${c}80)`,
      }} />
      {/* Mid marker */}
      <div style={{
        position: "absolute", left: `${midPct}%`, top: 3, width: 3, height: height + 6,
        background: c, borderRadius: 2, transform: "translateX(-1px)",
      }} />
      {/* External marker (industry) */}
      {markerPct !== null && (
        <>
          <div style={{
            position: "absolute", left: `${markerPct}%`, top: 2, width: 2, height: height + 8,
            background: markerColor || C.orange, borderRadius: 1, transform: "translateX(-1px)",
          }} />
          {markerLabel && (
            <div style={{
              position: "absolute", left: `${markerPct}%`, top: -8, transform: "translateX(-50%)",
              fontSize: 7, fontWeight: 600, color: markerColor || C.orange, whiteSpace: "nowrap",
            }}>
              {markerLabel}
            </div>
          )}
  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      <div style={{
        ...
        {centerLabel && (
          <div style={{ fontSize: size * 0.1, color: C.textDim, fontWeight: 600 }}>
            {centerLabel}
          </div>
        )}
      </div>
