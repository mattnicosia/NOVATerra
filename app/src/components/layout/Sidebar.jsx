...
function Sidebar() {
  ...
  const otherSessions = useUiStore(s => s.otherSessions);
      ...
      : syncStatus === "syncing"
        ...
        : syncStatus === "partial"
          ...
          : syncStatus === "error"
            ? P.orange
            : P.textDim;
  // Relative time for sync status
  const relativeTime = (() => {
    if (!syncLastFullAt) return syncLastAt || "";
    const diff = Date.now() - new Date(syncLastFullAt).getTime();
    if (diff < 30_000) return "just now";
    if (diff < 60_000) return "< 1 min ago";
    if (diff < 3_600_000) return `${Math.floor(diff / 60_000)} min ago`;
    return syncLastAt || ""; // Fall back to formatted time after 1 hour
  })();
  const statusLabel =
    syncStatus === "synced"
      ? `Synced${relativeTime ? ` ${relativeTime}` : ""}`
      : syncStatus === "syncing"
        ? "Syncing..."
        : syncStatus === "partial"
          ? `Partial sync${relativeTime ? ` ${relativeTime}` : ""}`
          : syncStatus === "error"
            ? "Tap to retry"
            : "Cloud sync";
          ...
          <div style={{ padding: `0 ${T.space[3]}px`, marginBottom: T.space[2] }}>
            ...
            <div
              ...
              style={{
                ...
                padding: `${T.space[1]}px ${open ? T.space[2] : 0}px`,
              }}
            ...
            >
              <div
                style={{
                  ...
                  flexShrink: 0,
                  boxShadow: syncStatus === "syncing" ? `0 0 6px ${P.accent}80` : "none",
            ...
            {open && otherSessions.length > 0 && (
              <div
                ...
                title={otherSessions.map(s => `${s.browser} on ${s.device}`).join(", ")}
              >
                <div style={{ width: 4, height: 4, borderRadius: 2, background: P.blue || P.accent, flexShrink: 0 }} />
                <span style={{ fontSize: 8, color: P.textDim, opacity: 0.7 }}>
                  Also open on {otherSessions[0]?.browser || "another device"}
                  {otherSessions.length > 1 ? ` +${otherSessions.length - 1}` : ""}
                </span>
              </div>
            )}
          </div>
            ...
            >
              <div
                style={{
                  ...
                  border: membership?.color
                    ? `1px solid ${membership.color}40`
                    : `1px solid ${P.borderAccent || P.border}`,
                  display: "flex",
                  alignItems: "center",
