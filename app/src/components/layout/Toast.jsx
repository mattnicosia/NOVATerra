...
function SingleToast({ toast, index, C, T }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        backdropFilter: C.noGlass ? "none" : "blur(16px)",
        WebkitBackdropFilter: C.noGlass ? "none" : "blur(16px)",
        border: `1px solid ${bg}40`,
        borderRadius: T.radius.md,
        boxShadow: `0 8px 32px ${bg}30, 0 0 20px ${bg}15`,
        overflow: "hidden",
        marginTop: index > 0 ? T.space[2] : 0,
      }}
    >
      <div
        style={{
  ...
  return (
    ...
    >
      <AnimatePresence mode="popLayout">
        {toasts.map((t, i) => (
          <SingleToast key={t.id} toast={t} index={i} C={C} T={T} />
        ))}
      </AnimatePresence>
    </div>
  );
