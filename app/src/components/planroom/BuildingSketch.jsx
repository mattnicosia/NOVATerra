...
export default function BuildingSketch({ outlines, floorAssignments, floors, project, C, T }) {
  const data = useMemo(() => {
    ...
    if (!basePoly && fc > 0) {
      ...
      const defaults = {
        "residential-single": 1800, "residential-multi": 4000, "commercial-office": 5000,
        "industrial": 8000, "retail": 3500, "healthcare": 6000, "education": 5000,
        "hospitality": 4000, "restaurant": 2000, "parking": 10000,
      };
    ...
    const floorPolys = elevs.map(elev =>
      centered.map(p => iso(p.x, elev, p.z))
    );
    ...
    for (let i = 0; i < n; i++) {
      ...
      const b0 = floorPolys[0][i], b1 = floorPolys[0][j];
      ...
      if (dot > 0) {
        ...
        for (let fi = 1; fi < elevs.length - 1; fi++) {
          const p0 = floorPolys[fi][i], p1 = floorPolys[fi][j];
    ...
    if (vertEdges.length > 0) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (dot <= 0 && prevDot > 0) {
          const b = floorPolys[0][i], t = floorPolys[floorPolys.length - 1][i];
    ...
    let bestFrontEdge = null, bestLen = 0;
    for (let i = 0; i < n; i++) {
      const j = (i + 1) % n;
      const dx = centered[j].x - centered[i].x;
      const dz = centered[j].z - centered[i].z;
      if (dz - dx > 0) { // front-facing
        const len = Math.sqrt(dx * dx + dz * dz);
        if (len > bestLen) { bestLen = len; bestFrontEdge = [i, j]; }
      }
    }
    if (bestFrontEdge) {
      ...
      const edgeLen = Math.sqrt(
        (centered[b].x - centered[a].x) ** 2 +
        (centered[b].z - centered[a].z) ** 2
      );
      dims.push({
        x1: p0.sx, y1: p0.sy, x2: p1.sx, y2: p1.sy,
        tx: (p0.sx + p1.sx) / 2, ty: (p0.sy + p1.sy) / 2 + 10,
    ...
    if (vertEdges.length > 0) {
      const rightVert = vertEdges.reduce((best, e) =>
        e.x1 > best.x1 ? e : best, vertEdges[0]);
      dims.push({
        x1: rightVert.x1 + 6, y1: rightVert.y1,
        x2: rightVert.x2 + 6, y2: rightVert.y2,
        tx: rightVert.x1 + 14,
    ...
    return {
      tier, vb, topFace, bottomFace, frontFaces, backEdges,
      vertEdges, floorLines, floorLabels, dims,
      polyW: Math.round(polyW), polyD: Math.round(polyD),
    };
  ...
  if (data.tier === 3) {
    return (
      <div style={{
        ...
        justifyContent: "center", padding: T.space[5], width: "100%",
      }}>
        <div style={{
          width: 44, height: 44, borderRadius: T.radius.md,
          background: `${C.textDim}08`, border: `2px dashed ${C.textDim}20`,
  ...
  return (
    <div style={{ width: "100%", position: "relative" }}>
      ...
      <div style={{
        ...
        background: `${C.textDim}08`, textTransform: "uppercase",
      ...
      >
        <defs>
          ...
          <pattern id="bldg-hatch-light" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(-45)">
        ...
        {data.backEdges.map((e, i) => (
          <line key={`be-${i}`}
            ...
            stroke={C.textDim} strokeWidth={0.4}
            strokeDasharray="2 3" strokeOpacity={0.2}
          />
        ))}
        ...
        {data.frontFaces.map((d, i) => (
          <path key={`sf-${i}`} d={d}
            fill="url(#bldg-hatch)" stroke={C.textDim} strokeWidth={0.8} strokeOpacity={0.5}
          />
        ))}

        {/* Top face — subtle accent fill */}
        <path d={data.topFace}
          fill={`${C.accent}10`} stroke={C.textDim} strokeWidth={1} strokeOpacity={0.6}
        />
        ...
        {data.vertEdges.map((e, i) => (
          <line key={`ve-${i}`}
            x1={e.x1} y1={e.y1} x2={e.x2} y2={e.y2}
            stroke={C.textDim} strokeWidth={1} strokeOpacity={0.6}
          />
        ))}
        ...
        {data.dims.map((d, i) => (
          <g key={`dim-${i}`}>
            ...
            <line x1={d.x1} y1={d.y1} x2={d.x2} y2={d.y2}
              stroke={C.accent} strokeWidth={0.5} strokeOpacity={0.4}
            />
