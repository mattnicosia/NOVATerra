...
export default function EstimateKPIStrip() {
  ...
  const totalCO2e = useMemo(() =>
    items.reduce((sum, item) => sum + getItemCO2e(item), 0),
  [items]);
  ...
  return (
    <div style={{
      ...
      <KPI label="% Priced" value={`${pricedPct}%`} icon={I.check} color={pricedPct === 100 ? C.green : C.orange} sub={`${pricedCount} of ${items.length}`} />
      <KPI label="$/SF" value={projectSF > 0 ? fmt(Math.round(perSF)) : "\u2014"} icon={I.ruler} color={C.cyan || C.accent} sub={projectSF > 0 ? `${nn(projectSF).toLocaleString()} SF` : "Set project SF"} />
      <KPI label="CO\u2082e" value={totalCO2e > 0 ? formatCarbon(totalCO2e) : "\u2014"} icon={I.layers} color={C.green} sub="kg estimated" />
    </div>
  );
}
