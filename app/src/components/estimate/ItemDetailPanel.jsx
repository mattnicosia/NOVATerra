...
const CO2E_TRADE = {
  ...
  '23': 'hvac', '26': 'electrical', '31': 'sitework', '32': 'sitework', '33': 'sitework',
};
    ...
    <div style={{ borderBottom: `1px solid ${C.border}30` }}>
      <button
        ...
        style={{
          ...
          fontSize: T.fontSize.xs, fontWeight: 700,
      ...
      >
        ...
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke={C.textDim} strokeWidth="2"
...
export default function ItemDetailPanel({ itemId, onClose, onNavigate }) {
  const C = useTheme();
  const T = C.T;
  ...
  const computedQty = item.formula?.trim()
    ? evalFormula(item.formula, (item.variables || []).filter(v => v.key).map(v => ({ name: v.key, value: nn(v.value) })), nn(item.quantity))
    : nn(item.quantity);
  ...
  const excludeItem = () => {
    ...
    useSpecsStore.getState().addExclusion({ text: excText, aiText: "", code: item.code, division: item.division, description: item.description, source: "estimate" });
    removeItem(item.id);
    ...
    }).catch(() => {}).finally(() => useSpecsStore.getState().setAiExclusionLoading(false));
  };
  ...
  const costField = (label, field) => {
    ...
    return (
      <div key={field} style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ width: 70, fontSize: T.fontSize.xs, fontWeight: 600, color: C.textDim, textTransform: "capitalize" }}>{label}</span>
        <input
          ...
          placeholder="0.00"
          style={nInp(C, {
            flex: 1, padding: "5px 8px", fontSize: T.fontSize.sm,
        ...
        <button
          ...
          style={{
            ...
            fontSize: 9, fontWeight: 800, textAlign: "center", lineHeight: "20px", padding: 0,
  ...
  return (
    ...
    >
      ...
      <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <input
              ...
              style={inp(C, {
                ...
                padding: "2px 0", fontSize: T.fontSize.md, fontWeight: 600,
              })}
            />
          </div>
          <button
            onClick={onClose}
            style={{
              width: 28, height: 28, border: `1px solid ${C.border}`, background: "transparent",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button
            ...
            style={{
              padding: "3px 8px", fontSize: T.fontSize.sm, fontWeight: 600,
          ...
          <span style={{ fontSize: T.fontSize.xs, color: C.textDim, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {item.division || ""}
          </span>
          {displayDir && (
            <span style={{
              fontSize: 9, fontWeight: 700, color: C.text,
              background: C.bg2, padding: "1px 6px", borderRadius: 3,
            }}>{displayDir}</span>
          )}
        </div>
      </div>
      ...
      <div style={{ padding: `${T.space[3]}px ${T.space[4]}px`, borderBottom: `1px solid ${C.border}30` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: T.fontSize.xs, fontWeight: 600, color: C.textDim, width: 70, textTransform: "uppercase" }}>Qty</span>
          <input
      ...
      {/* Pricing section */}
      <div style={{ padding: `${T.space[3]}px ${T.space[4]}px`, borderBottom: `1px solid ${C.border}30`, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: T.fontSize.xs, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 2 }}>
          Pricing (per unit)
        </div>
      ...
      <div style={{
        ...
        <button onClick={() => setPricingModal(item)}
          style={bt(C, { padding: "7px 10px", background: `${C.accent}12`, border: `1px solid ${C.accent}30`, color: C.accent, justifyContent: "center" })}>
        ...
        <button onClick={() => {
          ...
          style={bt(C, { padding: "7px 10px", background: `${C.green}12`, border: `1px solid ${C.green}30`, color: C.green, justifyContent: "center" })}>
          <Ic d={I.send} size={12} /> Send to DB
        </button>
        <button onClick={() => { duplicateItem(item.id); showToast("Item duplicated"); }}
          style={bt(C, { padding: "7px 10px", background: `${C.text}08`, border: `1px solid ${C.border}`, color: C.textMuted, justifyContent: "center" })}>
          <Ic d={I.copy} size={12} /> Duplicate
        </button>
        <button onClick={excludeItem}
          style={bt(C, { padding: "7px 10px", background: `${C.orange}12`, border: `1px solid ${C.orange}30`, color: C.orange, justifyContent: "center" })}>
          Exclude
        </button>
      </div>
      ...
      {/* Sub-items */}
      <Section title={`Sub-items (${sItemList.length})`} icon={I.assembly} color={sItemList.length > 0 ? C.accent : undefined} defaultOpen={sItemList.length > 0} C={C}>
        {sItemList.length > 0 && (
          <div style={{ fontSize: T.fontSize.xs, color: subItemTotal > 0 && Math.abs(subItemTotal - unitTotal) > 0.01 ? C.orange : C.green, fontWeight: 600, marginBottom: 6 }}>
        ...
        {sItemList.map(si => {
          ...
          return (
            <div key={si.id} style={{ display: "flex", flexDirection: "column", gap: 3, padding: "6px 0", borderBottom: `1px solid ${C.border}20` }}>
              <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                <input value={si.desc} onChange={e => updateSubItem(item.id, si.id, "desc", e.target.value)}
                  placeholder="Description..." style={inp(C, { flex: 1, background: "transparent", border: "1px solid transparent", padding: "2px 4px", fontSize: 11 })} />
                <button onClick={() => removeSubItem(item.id, si.id)}
                  style={{ width: 18, height: 18, border: "none", background: "transparent", borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                  <Ic d={I.x} size={9} color={C.red} />
                </button>
              </div>
              <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                <input type="number" value={si.factor || 1} onChange={e => updateSubItem(item.id, si.id, "factor", parseFloat(e.target.value) || 1)}
                  placeholder="1" style={nInp(C, { width: 40, background: "transparent", border: "1px solid transparent", padding: "2px", fontSize: 10, textAlign: "center" })} />
                <input value={si.unit} onChange={e => updateSubItem(item.id, si.id, "unit", e.target.value)}
                  style={inp(C, { width: 36, background: "transparent", border: "1px solid transparent", padding: "2px", fontSize: 10, textAlign: "center" })} />
                <input type="number" value={si.m} onChange={e => updateSubItem(item.id, si.id, "m", parseFloat(e.target.value) || 0)}
                  placeholder="M" style={nInp(C, { flex: 1, background: "transparent", border: "1px solid transparent", padding: "2px", fontSize: 10 })} />
                <input type="number" value={si.l} onChange={e => updateSubItem(item.id, si.id, "l", parseFloat(e.target.value) || 0)}
        ...
        <button onClick={() => addSubItem(item.id)}
          style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 0", background: "transparent", color: C.accent, cursor: "pointer", fontSize: 10, fontWeight: 600, border: "none", marginTop: 4 }}>
      ...
      <Section title="Specification" icon={I.plans} color={item.specText ? C.purple : undefined} C={C}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div>
            <div style={{ fontSize: T.fontSize.xs, color: C.textDim, fontWeight: 600, marginBottom: 2, textTransform: "uppercase" }}>Spec Section</div>
            <input value={item.specSection || ""} onChange={e => updateItem(item.id, "specSection", e.target.value)}
              placeholder="e.g. 07 61 00" style={inp(C, { fontSize: T.fontSize.sm, padding: "4px 8px" })} />
          </div>
          <div>
            <div style={{ fontSize: T.fontSize.xs, color: C.textDim, fontWeight: 600, marginBottom: 2, textTransform: "uppercase" }}>Written Specification</div>
            <textarea value={item.specText || ""} onChange={e => updateItem(item.id, "specText", e.target.value)}
              ...
              rows={2} style={inp(C, { fontSize: T.fontSize.sm, padding: "4px 8px", resize: "vertical", minHeight: 36 })} />
          </div>
        </div>
      </Section>
      ...
      <Section title="Metadata" icon={I.settings} C={C}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <select value={displayDir} onChange={e => { updateItem(item.id, "directive", e.target.value); updateItem(item.id, "directiveOverride", true); }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button
              ...
              style={bt(C, {
                padding: "4px 10px", fontSize: T.fontSize.xs,
      ...
      <div style={{ padding: `${T.space[4]}px`, marginTop: "auto" }}>
        <button onClick={() => { removeItem(item.id); onClose(); showToast("Item deleted"); }}
          style={bt(C, {
            width: "100%", padding: "8px", justifyContent: "center",
