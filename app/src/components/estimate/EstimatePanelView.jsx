...
export default function EstimatePanelView({ onSelectItem, selectedItemId }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {/* Mini toolbar */}
      <div style={{ padding: "4px 8px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
        {/* View mode toggle */}
        <div style={{ display: "flex", gap: 0, background: C.bg2, borderRadius: 4, padding: 1 }}>
          {[
            ...
            <button key={v.key} onClick={() => setEstViewMode(v.key)} style={{
              padding: "2px 7px", fontSize: 8, fontWeight: viewMode === v.key ? 700 : 500,
        ...
        {/* Search */}
        <input value={estSearch || ""} onChange={e => setEstSearch(e.target.value)}
          placeholder="Search..."
          style={inp(C, { flex: 1, background: C.bg2, border: `1px solid ${C.border}`, padding: "3px 6px", fontSize: 9, borderRadius: 4, minWidth: 60 })} />
      </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", minHeight: 0 }}>
          <div style={{ padding: "4px 6px" }}>
            {sortedGroups.map(([gk, group]) => {
              ...
              return (
                <div key={gk} style={{ marginBottom: 4, border: `1px solid ${C.border}`, borderRadius: T.radius.sm, overflow: "hidden" }}>
                  {/* Group Header */}
                  <div className="nav-item" onClick={() => toggleExpandedDiv(gk)}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 8px", background: `linear-gradient(180deg, ${C.bg1}, ${C.bg2}40)`, cursor: "pointer" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, color: C.text, fontFeatureSettings: "'tnum'" }}>{gkLabel}</span>
                      <span style={{ fontSize: 9, color: C.textDim, background: C.bg, padding: "0 4px", borderRadius: 6 }}>{skCount}</span>
                    </div>
                    <span style={{ fontSize: 10, color: C.textMuted, fontFeatureSettings: "'tnum'", fontFamily: "'DM Sans',sans-serif" }}>{fmt(skTotal)}</span>
                  </div>
                  ...
                  {isExpanded && (
                    <div style={{
                      ...
                      background: C.bg2, padding: "4px 6px", fontSize: 7, fontWeight: 600, color: C.textDim,
                      textTransform: "uppercase", letterSpacing: 0.6, gap: 3,
                    }}>
                  ...
                  {isExpanded && skItems.map((item, rowIdx) => {
                    ...
                    return (
                      <div key={item.id} onClick={() => onSelectItem?.(item.id)}
                        onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = `${C.accent}08`; }}
                        ...
                        {/* Code */}
                        <div style={{ width: 60, fontSize: 9, fontWeight: 600, color: item.code ? C.text : C.textDim, fontFeatureSettings: "'tnum'" }}>
                          {item.code || "\u2014"}
                        </div>
                        {/* Description */}
                        <div style={{ flex: 1, minWidth: 80 }}>
                          <input value={item.description} onChange={e => { e.stopPropagation(); updateItem(item.id, "description", e.target.value); }}
                            ...
                            style={inp(C, { background: "transparent", border: "1px solid transparent", padding: "2px 3px", fontSize: 9 })} />
                        </div>
                        {/* Qty */}
                        <div style={{ width: 48 }} onClick={e => e.stopPropagation()}>
                          <input type="number" value={item.quantity} onChange={e => updateItem(item.id, "quantity", e.target.value)}
                            placeholder="0" style={nInp(C, { background: "transparent", border: "1px solid transparent", padding: "2px 2px", fontSize: 9 })} />
                        </div>
                        {/* Unit */}
                        <div style={{ width: 32 }} onClick={e => e.stopPropagation()}>
                          <select value={item.unit} onChange={e => updateItem(item.id, "unit", e.target.value)}
                            style={inp(C, { background: "transparent", border: "1px solid transparent", padding: "2px 0", fontSize: 8 })}>
                          ...
                          return (
                            <div key={f} style={{ width: 56 }} onClick={e => e.stopPropagation()}>
                              <input type="text" inputMode="decimal" value={displayVal}
                                ...
                                placeholder="0"
                                style={nInp(C, { background: "transparent", border: "1px solid transparent", padding: "2px 1px", fontSize: 9, textAlign: "right" })} />
                            </div>
                          );
                        })}
                        {/* Total */}
      ...
      {items.length > 0 && (
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 800, color: C.text, fontFeatureSettings: "'tnum'", fontFamily: "'DM Sans',sans-serif" }}>
            {fmt(totals.grand)}
          </span>
        </div>
      )}
