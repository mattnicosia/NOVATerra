...
Your analysis should be direct, actionable, and specific to this project. Reference actual line items, quantities, and costs from the estimate data. Flag anything that looks wrong.`,
        messages: [
          {
            role: "user",
            content: `Perform a Bid Day Intelligence analysis on this estimate.
    ...
    <>
      ...
      <div
        style={{
          ...
          boxShadow: `0 24px 64px rgba(0,0,0,0.3), 0 0 40px rgba(0,0,0,0.15)`,
          zIndex: 901,
          display: "flex",
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: C.textDim }}>
                {project.name} • {fmt(totals.grand)}
                {sf > 0 ? ` • ${fmt2(costPerSF)}/SF` : ""}
              </div>
            </div>
          </div>
        ...
        >
          {[
            ...
            >
              <div
                style={{ fontSize: 8, fontWeight: 600, color: s.color, textTransform: "uppercase", letterSpacing: 0.5 }}
              >
                {s.label}
              </div>
              <div style={{ fontSize: 14, fontWeight: 800, color: s.color, fontFamily: T.font.sans }}>
                {s.value}
              </div>
            </div>
          ))}
          ...
          {!analysis && !loading && (
            <div style={{ textAlign: "center", padding: "30px 0" }}>
              ...
              <div style={{ fontSize: 12, color: C.textDim, lineHeight: 1.5, maxWidth: 400, margin: "0 auto 20px" }}>
                AI will benchmark your costs against industry averages, flag pricing outliers, identify scope gaps, and
                provide bid strategy recommendations.
              </div>
              <button
                onClick={runAnalysis}
                style={bt(C, {
              ...
              {analysis.split("\n").map((line, i) => {
                if (line.startsWith("## "))
                  return (
                    <div
                      key={i}
                      style={{ fontSize: 15, fontWeight: 800, color: C.accent, marginTop: 16, marginBottom: 4 }}
                    >
                      {line.slice(3)}
                    </div>
                  );
                if (line.startsWith("**") && line.endsWith("**"))
                  return (
                    <div key={i} style={{ fontSize: 13, fontWeight: 700, color: C.text, marginTop: 12 }}>
                      {line.replace(/\*\*/g, "")}
                    </div>
                  );
