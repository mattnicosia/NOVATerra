...
export default function ScopeGapReport({ estimateItems, parsedProposal, projectName, packageName, subName }) {
  ...
  const report = useMemo(
    () => analyzeGaps(estimateItems, parsedProposal),
    [estimateItems, parsedProposal],
  );

  const scoreColor = report.coverageScore >= 80 ? '#30D158'
    : report.coverageScore >= 60 ? '#FF9F0A'
    : '#FF453A';
    ...
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        <div style={{ color: C.textMuted, fontSize: 13 }}>
          No estimate items to compare against this proposal.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {/* Score */}
        <div style={{
          flex: '0 0 auto', width: 120, textAlign: 'center',
          ...
          <div style={{ color: C.textMuted, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 6 }}>
            Coverage
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          <StatCard label="Missing Scope" value={report.missingFromProposal.length} suffix=" div" color="#FF453A" C={C} />
      ...
      {report.totalExposure > 0 && (
        <div style={{
          ...
          background: 'rgba(255,69,58,0.06)', border: '1px solid rgba(255,69,58,0.12)',
        }}>
            ...
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <span style={{
                    ...
                    padding: '1px 6px', borderRadius: 4, fontSize: 11, fontWeight: 600, marginRight: 8,
                  }}>
                    Div {gap.division}
                  </span>
              ...
              <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
                {gap.estimateItems.slice(0, 5).map((item, j) => (
                  <div key={j} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.textMuted, paddingLeft: 8 }}>
                    <span>{item.code ? `${item.code} — ` : ''}{item.description}</span>
                    <span>{item.quantity} {item.unit} · {fmt(item.total)}</span>
                  </div>
                ))}
          ...
          {report.exclusionConflicts.map((conflict, i) => (
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{ color: '#FF9F0A', fontSize: 13, fontWeight: 600 }}>{fmt(conflict.estimatedExposure)}</span>
              </div>
              <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <tbody>
              {report.quantityMismatches.map((mm, i) => (
                <tr key={i} style={{ borderBottom: `1px solid ${C.border}10` }}>
                  <td style={tdS(C)}>
                    <span style={{ background: '#64D2FF20', color: '#64D2FF', padding: '1px 6px', borderRadius: 4, fontSize: 10, fontWeight: 600 }}>
                      {mm.division}
                    </span>
                  </td>
                  <td style={{ ...tdS(C), textAlign: 'left', color: C.text }}>
                    {mm.estimateItem}
                  </td>
                  <td style={tdS(C)}>{mm.estQty} {mm.unit}</td>
                  <td style={tdS(C)}>{mm.propQty} {mm.unit}</td>
                  <td style={{
                    ...tdS(C),
                    color: mm.pctDiff > 0 ? '#30D158' : '#FF453A',
                    fontWeight: 600,
                  }}>
                    {mm.pctDiff > 0 ? '+' : ''}{mm.pctDiff}%
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        >
          {report.extraInProposal.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {item.csiCode && (
                  <span style={{ background: '#BF5AF220', color: '#BF5AF2', padding: '1px 6px', borderRadius: 4, fontSize: 10, fontWeight: 600 }}>
                    {item.csiCode}
                  </span>
                )}
        ...
        {narrative ? (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ color: C.accent, fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                NOVA Analysis
              </span>
          ...
          <div style={{ textAlign: 'center' }}>
            {narrativeError && (
              <div style={{ color: '#FF453A', fontSize: 12, marginBottom: 6 }}>{narrativeError}</div>
            )}
            <button
            ...
            >
              <Ic d={I.ai} size={14} color={C.accent} />
              {narrativeLoading ? 'Analyzing...' : narrativeError ? 'Retry AI Risk Analysis' : 'Generate AI Risk Analysis'}
            </button>
          </div>
        )}
      </div>
...
function StatCard({ label, value, suffix, color, C }) {
  return (
    <div style={{
      ...
      <div style={{ color: C.textMuted, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.3, marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      >
        ...
        <Ic d={I.chevron} size={12} color={C.textMuted}
...
const thS = (C) => ({
  padding: '6px', textAlign: 'right', fontSize: 10,
  fontWeight: 600, color: C.textDim, textTransform: 'uppercase', letterSpacing: 0.3,
});

const tdS = (C) => ({
  padding: '6px', textAlign: 'right', color: C.textMuted,
});
