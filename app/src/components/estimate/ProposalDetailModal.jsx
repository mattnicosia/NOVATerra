      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        ...
        {pd.totalBid && (
          <div style={{
            ...
            <div style={{ color: '#8E8E93', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              Total Bid
            </div>
            <div style={{ color: '#30D158', fontSize: 20, fontWeight: 700 }}>
              {fmt(pd.totalBid)}
            </div>
          </div>
        )}
      ...
      <div style={{ maxHeight: 480, overflowY: 'auto' }}>
              ...
              <tbody>
                {lineItems.map((item, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.border}15` }}>
                    ...
                    <td style={{ ...tdStyle(C), textAlign: 'left', color: C.text }}>
                      {item.description}
                    </td>
          ...
          <Section title="Alternates" count={alternates.length} color="#FF9F0A">
            {alternates.map((alt, i) => (
              <div key={i} style={{
                ...
                padding: '6px 0', borderBottom: `1px solid ${C.border}10`,
              }}>
                ...
                <span style={{
                  ...
                  {alt.type === 'deduct' ? '−' : '+'}{alt.amount ? fmt(alt.amount) : 'TBD'}
                </span>
              </div>
            ))}
          </Section>
        ...
        {(pd.paymentTerms || pd.validityPeriod || pd.scheduleDuration || pd.bondIncluded != null) && (
          <Section title="Additional Details">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              {pd.bondIncluded != null && <DetailCell label="Bond" value={pd.bondIncluded ? 'Included' : 'Not Included'} />}
              {pd.insuranceIncluded != null && <DetailCell label="Insurance" value={pd.insuranceIncluded ? 'Included' : 'Not Included'} />}
            </div>
          </Section>
        )}
        ...
        {proposal.parseStatus === 'error' && (
          <div style={{
            ...
            color: '#FF453A', fontSize: 13, marginTop: 12,
          }}>
        ...
        {proposal.parseStatus === 'pending' && (
          <div style={{
            ...
            color: C.accent, fontSize: 13, marginTop: 12,
          }}>
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{
        ...
        letterSpacing: 0.5, color: color || C.textMuted, marginBottom: 8,
      }}>
        {title}
...
function DetailCell({ label, value }) {
  ...
  return (
    <div style={{ padding: '8px 10px', borderRadius: 6, background: 'rgba(255,255,255,0.03)' }}>
      <div style={{ color: C.textDim, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.3, marginBottom: 2 }}>
        {label}
      </div>
...
const tdStyle = (C) => ({
  padding: '8px 6px', textAlign: 'right', color: C.textMuted,
});
