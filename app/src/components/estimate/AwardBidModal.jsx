    ...
    try {
      ...
      for (const inv of pkgInvites) {
        updateInvitationStatus(
          bidPackage.id,
          inv.id,
          inv.id === selectedId ? 'awarded' : 'not_awarded',
        );
      }
      ...
      {step === 'select' ? (
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {parsedOptions.map((opt, i) => {
                const scoreColor = opt.coverageScore >= 80 ? '#30D158' : opt.coverageScore >= 60 ? '#FF9F0A' : '#FF453A';
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, marginTop: 3, fontSize: 11, color: C.textMuted }}>
                        {opt.coverageScore != null && (
                          <span>Coverage: <span style={{ color: scoreColor, fontWeight: 600 }}>{opt.coverageScore}%</span></span>
                        )}
                        {opt.exposure > 0 && (
                          <span>Exposure: <span style={{ color: '#FF453A', fontWeight: 600 }}>{fmt(opt.exposure)}</span></span>
                        )}
                        {opt.exclusionCount > 0 && (
                          <span>{opt.exclusionCount} exclusion{opt.exclusionCount !== 1 ? 's' : ''}</span>
                        )}
                      </div>
                    </div>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ color: C.textMuted, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
              Awarding to
            </div>
            ...
            {selected?.email && (
              <div style={{ color: C.textMuted, fontSize: 12, marginTop: 4 }}>{selected.email}</div>
            )}
          </div>

          <div style={{
            padding: '12px 14px', borderRadius: 10,
            background: 'rgba(124,92,252,0.06)', border: `1px solid ${C.accent}20`,
            color: C.textMuted, fontSize: 12, lineHeight: 1.6, marginBottom: 16,
          }}>
            <div style={{ fontWeight: 600, color: C.text, marginBottom: 6 }}>This will:</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <div>1. Send an award notification email to <strong style={{ color: C.text }}>{selected?.name}</strong></div>
              <div>2. Send constructive feedback to {losers.length} other bidder{losers.length !== 1 ? 's' : ''}</div>
              <div>3. Close the bid package</div>
            </div>
          </div>

          {notes && (
            <div style={{ padding: '10px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', marginBottom: 16 }}>
              <div style={{ color: C.textDim, fontSize: 10, fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Notes</div>
