...
function DimensionCard({ v, idx, C, updateVariable, removeVariable, nInp }) {
  ...
  return (
    <div
      ...
      style={{
        width: 90, minHeight: 86, padding: "10px 8px 8px",
        borderRadius: 10,
    ...
    >
      {/* Icon */}
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.7 }}>
        <path d={icon} />
      </svg>
      ...
      {isCustom ? (
        <input value={v.key} onChange={e => updateVariable(idx, "key", e.target.value)}
          placeholder="Name"
          style={{
            width: 70, padding: "1px 2px", fontSize: 10, fontWeight: 700,
            color, textTransform: "uppercase", letterSpacing: 0.6, textAlign: "center",
            background: "transparent", border: `1px solid ${color}20`, borderRadius: 4,
      ...
      {/* Unit */}
      {unitLabel && (
        <div style={{ fontSize: 10, color: C.textDim, fontWeight: 500 }}>{unitLabel}</div>
      )}
...
export default function ComputationChain({ item }) {
  ...
  return (
    <div style={{
      padding: "16px 20px 16px 24px", background: C.bg1,
      borderBottom: `1px solid ${C.border}`, borderLeft: `4px solid ${chainColor}`,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: chainColor, textTransform: "uppercase", letterSpacing: 0.8 }}>
              Dimension Engine
            </div>
            <div style={{ fontSize: 11, color: C.textDim, marginTop: 1, maxWidth: 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {fresh.description}
            </div>
          </div>
        </div>
        <button className="icon-btn" onClick={() => setEstShowVars(null)}
          style={{ width: 24, height: 24, border: `1px solid ${C.border}`, background: C.bg2, color: C.textDim, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Ic d={I.x} size={11} />
        </button>
      </div>
      ...
      {hasFormula && narrative && (
        <div style={{
          padding: "10px 14px", background: `${chainColor}06`, borderRadius: 8,
          marginBottom: 14, fontSize: 13, color: C.text, lineHeight: 1.8,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <div style={{
          ...
          border: `1.5px solid ${C.purple}25`, background: `${C.purple}08`,
        ...
        {hasFormula && (
          <>
            ...
            <div style={{
              ...
              border: `1.5px solid ${chainColor}35`, background: `${chainColor}10`,
      ...
      {(variables.length > 0 || true) && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>
            Dimensions
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "stretch" }}>
            {variables.map((v, idx) => (
              <DimensionCard
                key={idx} v={v} idx={idx} C={C}
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                style={{
                  width: 90, minHeight: 86, padding: "10px 8px",
                  borderRadius: 10, border: `1.5px dashed ${C.border}`,
              ...
              {showAddMenu && (
                <div style={{
                  ...
                  boxShadow: T.shadow.lg, padding: 6, minWidth: 160, maxHeight: 280, overflowY: "auto",
                }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: C.orange, textTransform: "uppercase", letterSpacing: 0.6 }}>Formula</div>
          <input
            value={formula}
          ...
          />
          {hasFormula && (
            <button onClick={() => { updateItem(fresh.id, "formula", ""); }}
              style={{ padding: "4px 8px", fontSize: 10, fontWeight: 600, border: `1px solid ${C.border}`, background: "transparent", color: C.textDim, borderRadius: 5, cursor: "pointer" }}>
              Clear
            </button>
          )}
        </div>
        <div style={{ fontSize: 10, color: C.textDim, lineHeight: 1.5 }}>
          Use <strong style={{ color: C.purple }}>Qty</strong> for base quantity. Reference dimensions by name. Supports + - * / and parentheses.
        </div>
      </div>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.6 }}>Scenarios</div>
          ...
          {SCENARIOS.filter(s => s.cat === scenarioCat).map((s, i) => (
            <button key={i} onClick={() => applyScenario(s)}
              style={{
                ...
                color: C.accent, borderRadius: 6, cursor: "pointer",
                transition: "all 0.1s",
              }}>
              {s.label}
            </button>
