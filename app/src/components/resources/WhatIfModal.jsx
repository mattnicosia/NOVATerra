...
export default function WhatIfModal({ workload, onClose }) {
  ...
  const rankings = useMemo(() => {
    ...
    return estimatorRows.map(row => {
      ...
      const avgRemaining = relevantDays.length > 0
      ...
      if (utilization < 0.5) { score += 2; reasons.push("Light load"); }
      else if (utilization < 0.7) { score += 1; }
      else if (utilization > 0.9) { score -= 2; reasons.push("Near capacity"); }

      // Current estimate count
      if (row.estimates.length <= 2) { score += 1; }
      else if (row.estimates.length >= 5) { score -= 1; reasons.push(`${row.estimates.length} active bids`); }
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: T.space[4] }}>
          <div>
            ...
            <div style={{ fontSize: T.fontSize.xs, color: C.textDim }}>Simulate adding a new estimate to see team impact</div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: T.space[3], marginBottom: T.space[4] }}>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={{ fontSize: 9, fontWeight: 700, color: C.textDim, textTransform: "uppercase", display: "block", marginBottom: 4 }}>
              Project Name
            </label>
            <input
              value={name}
        ...
        {rankings.length > 0 && (
          <>
            <div style={{
              ...
              <div style={{ fontSize: T.fontSize.base, fontWeight: 700, color: canTakeOn ? "#30D158" : "#FF3B30", marginBottom: 4 }}>
                {canTakeOn ? "Yes — team can absorb this" : "Risk — may create conflicts"}
              </div>
              <div style={{ fontSize: T.fontSize.xs, color: C.textDim }}>
                {bestFit && (
                  <>
                    Best fit: <strong>{bestFit.name}</strong> ({bestFit.totalRemaining}h available, {bestFit.activeCount} active bids)
                    {bestFit.daysNeeded > bestFit.daysAvailable && (
                      <span style={{ color: "#FF9500" }}> — needs {bestFit.daysNeeded} work days but only {bestFit.daysAvailable} available</span>
                    )}
                  </>
                )}
              </div>
              ...
              {rankings.map((r, i) => (
                <div
                  ...
                  style={{
                    ...
                    background: i === 0 ? `${r.canFit ? "#30D158" : "#FF3B30"}08` : (C.isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)"),
