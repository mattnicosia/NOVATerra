...
export default function PdfExport({ workload }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const estimatorSections = estimatorRows.map(row => {
        ...
        return `
          <div style="margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
              <div style="width:20px;height:20px;border-radius:50%;background:${row.color || "#A78BFA"};display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:700;">
                ${(row.name || "?")[0]}
              </div>
      ...
      const alertSection = warnings.length > 0 ? `
        <div style="margin-top:16px;">
          ...
          ${warnings.slice(0, 8).map(w => {
            ...
            else if (w.type === "predicted_overload") msg = `${w.estimator} predicted overload in ${w.daysFromNow} days`;
            else if (w.type === "load_imbalance") msg = `Load imbalance: ${w.overloaded.name} vs ${w.underloaded.name}`;
