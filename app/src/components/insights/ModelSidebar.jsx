...
import MaterialPicker from "@/components/insights/MaterialPicker";
import { exportModelFromStore } from "@/utils/modelExport";
...
export default function ModelSidebar() {
  ...
  const setViewMode = useDrawingPipelineStore(s => s.setViewMode);
  const ifcLoaded = useDrawingPipelineStore(s => s.ifcLoaded);
  const autoGenerated = useDrawingPipelineStore(s => s.autoGenerated);
  ...
  const floorHeights = useDrawingPipelineStore(s => s.floorHeights);
  const levels = useDrawingPipelineStore(s => s.levels);
  const hiddenFloors = useDrawingPipelineStore(s => s.hiddenFloors);
  const toggleFloorVisibility = useDrawingPipelineStore(s => s.toggleFloorVisibility);
  const showAllFloors = useDrawingPipelineStore(s => s.showAllFloors);
  const sectionPlaneY = useDrawingPipelineStore(s => s.sectionPlaneY);
  const setSectionPlaneY = useDrawingPipelineStore(s => s.setSectionPlaneY);
  const xrayMode = useDrawingPipelineStore(s => s.xrayMode);
  const setXrayMode = useDrawingPipelineStore(s => s.setXrayMode);
  const specOverrides = useDrawingPipelineStore(s => s.specOverrides);
  const setSpecOverride = useDrawingPipelineStore(s => s.setSpecOverride);
  ...
  const handleFloorHeightChange = useCallback(e => {
    ...
    if (v >= 6 && v <= 30) useDrawingPipelineStore.getState().setFloorHeight(v);
  }, []);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: T.space[3], height: "100%", overflowY: "auto" }}>
      ...
      {hasOutlines && (
        <div style={{ ...card(C), padding: T.space[3] }}>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 3, flexShrink: 0 }}>
                  <input
                    ...
                    onChange={e => {
                      const v = parseFloat(e.target.value);
                      if (v >= 6 && v <= 40) useDrawingPipelineStore.getState().setFloorHeightFor(f.label, v);
                    }}
                    min={6}
                    max={40}
                    step={1}
          ...
          <div style={{ display: "flex", gap: 4, marginTop: T.space[2] }}>
            <button
              onClick={() => {
                useDrawingPipelineStore.setState({ outlines: {}, coverageCells: [], coverageStats: null });
              }}
              style={{
                ...bt(C),
                padding: "4px 8px",
      ...
      <div style={{ ...card(C), padding: T.space[3] }}>
        ...
        {elements.length > 0 && (
          <div style={{ display: "flex", gap: 4, marginTop: T.space[2] }}>
            <button
              onClick={() => exportModelFromStore("glb").catch(err => console.error("Export failed:", err))}
              style={{
                ...bt(C),
                padding: "5px 8px",
                fontSize: T.fontSize.xs,
            ...
            >
              <Ic d={I.download || I.save} size={10} color={C.accent} />
              Export GLB
            </button>
            <button
              onClick={() => exportModelFromStore("gltf").catch(err => console.error("Export failed:", err))}
              style={{
                ...bt(C),
                padding: "5px 8px",
                fontSize: T.fontSize.xs,
            ...
            >
              <Ic d={I.download || I.save} size={10} color={C.textDim} />
              Export glTF
            </button>
          </div>
        )}
      ...
      {selected ? (
        <div style={{ ...card(C), padding: T.space[3], border: `1px solid ${C.accent}40` }}>
          ...
          {/* ── Material Assignment ─────────────────────── */}
          <MaterialPicker element={selected} />
