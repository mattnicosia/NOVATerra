...
function returningFields() {
  return [
    ...
    {
      group: "Misc",
      fields: [{ label: "Typewriter speed", path: "typewriterSpeed", type: "number", suffix: "ms/char" }],
    },
  ];
}
...
export default function NovaScriptEditor() {
  ...
  const handleAddStep = useCallback((section, arrayPath) => {
    const template =
      section === "TOUR" ? { ...DEFAULT_TOUR_STEP } : { ...DEFAULT_SETUP_STEP, id: `step-${Date.now()}` };
        ...
        <div>
          ...
          {field.type === "number" && (
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              />
              {field.suffix && <span style={{ fontSize: 10, color: C.textMuted }}>{field.suffix}</span>}
            </div>
          )}
          ...
          {field.type === "color" && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input
                ...
                value={val || ""}
                onChange={e => handleChange(section, field.path, e.target.value, "text")}
                style={inp(C, { padding: "5px 10px", fontSize: 12, fontFamily: T.font.sans })}
              />
            </div>
          )}
          ...
          {(field.hint || isFn) && (
            <div style={hintStyle}>{field.hint || (isFn ? "Use {name} or {n} for dynamic values" : "")}</div>
          )}
        </div>
      </div>
    );
      ...
      {groups.map((g, gi) => {
        ...
        return (
          <div key={`${activeSection}-${gi}-${tick}`}>
            <div style={groupHeaderStyle(isExpanded)} onClick={() => setExpandedGroup(isExpanded ? null : gi)}>
            ...
            {isExpanded && <div style={groupBodyStyle}>{g.fields.map(f => renderField(activeSection, f))}</div>}
          </div>
        );
      })}
      ...
      {activeSection === "SOUNDS" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ fontSize: 11, color: C.textDim, lineHeight: 1.5 }}>
            Upload your own audio files to replace the default synthesized sounds. Supported formats: MP3, WAV, OGG. Max
            15 MB per file.
          </div>
          {AUDIO_SLOTS.map(slot => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <input
                      ...
                      style={{ display: "none" }}
                      onChange={e => {
                        handleAudioUpload(slot.key, e.target.files?.[0]);
                        e.target.value = "";
                      }}
                    />
                  </div>
                ...
                {meta && (
                  <div style={{ fontSize: 9, color: C.textMuted, fontFamily: T.font.sans, marginTop: 6 }}>
