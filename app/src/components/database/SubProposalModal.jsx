...
export default function SubProposalModal({ onClose }) {
  ...
  const pricingLoc = pricingZip?.length >= 3
  ...
  const applyNormalization = useCallback((items, loc, shouldNormalize) => {
    if (!shouldNormalize || loc.source === "none") {
      return items.map(item => ({ ...item, _normalized: false, _originalMaterial: undefined, _originalLabor: undefined, _originalEquipment: undefined }));
    }
  ...
  const reNormalize = useCallback((newZip, newNormalize) => {
    if (rawItems.length === 0) return;
    const loc = newZip?.length >= 3 ? resolveLocationFactors(newZip) : { mat: 1, lab: 1, equip: 1, label: "National Average", source: "none" };
...
[{"name":"Concrete Footings","unit":"CY","material":0,"labor":0,"equipment":0,"subcontractor":185.00,"quantity":45,"specSection":"03.310","specText":"4000 PSI normal weight, air entrained","trade":"concrete","subName":"ABC Concrete Inc.","projectZip":"10001"}]`;
      ...
      if (isPdf) {
        userContent = [
          pdfBlock(pdfBase64),
          { type: "text", text: `Extract all line items with unit rates from this subcontractor proposal.${subHint}${tradeHint} Return as JSON array.` },
        ];
      } else {
  ...
  const subAlreadyExists = subName && subcontractors.some(s =>
    (s.company || "").toLowerCase().trim() === subName.toLowerCase().trim()
  );


  const importSelected = () => {
    const userName = authUser?.user_metadata?.full_name || project?.estimator || (estimators.length > 0 ? estimators[0].name : "");
    ...
    for (const item of toImport) {
      ...
      addElement({
        code: item.specSection || "", name: item.name, unit: item.unit,
        material: item.material, labor: item.labor,
        equipment: item.equipment, subcontractor: item.subcontractor,
        directive: dir, trade: item.trade || trade || "",
        addedBy: userName, addedDate: new Date().toLocaleDateString(),
        specVariants: [], specText: item.specText || "",
  ...
  const modal = { background: C.bg, border: `1px solid ${C.glassBorder || 'rgba(255,255,255,0.08)'}`, borderRadius: 12, width: step === "review" ? 1160 : 520, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 16px 48px rgba(0,0,0,0.4), 0 0 40px rgba(0,0,0,0.15)", animation: "modalEnter 300ms cubic-bezier(0.16, 1, 0.3, 1) both", transition: "width 0.2s ease" };
      ...
      <div style={modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg, ${C.accent}20, ${C.purple}20)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Ic d={I.upload} size={16} color={C.accent} />
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Import Sub Proposal</div>
              <div style={{ fontSize: 10, color: C.textMuted }}>Upload a subcontractor proposal to extract unit rates</div>
            </div>
          </div>
        ...
        {step === "upload" && (
          <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, display: "block", marginBottom: 4 }}>Subcontractor Name (optional — AI will detect)</label>
              <input value={subName} onChange={e => setSubName(e.target.value)} placeholder="e.g. ABC Electrical"
                style={inp(C, { fontSize: 12, padding: "8px 10px" })} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, display: "block", marginBottom: 4 }}>Trade (optional — AI will detect)</label>
              <select value={trade} onChange={e => setTrade(e.target.value)}
                style={inp(C, { fontSize: 12, padding: "8px 10px" })}>
                <option value="">Auto-detect from proposal</option>
                {TRADE_OPTIONS.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
              </select>
            </div>
            ...
            <div
              ...
              style={{
                ...
                background: file ? `${C.accent}08` : C.bg2, transition: "all 0.15s",
              }}>
              <input ref={fileRef} type="file" accept=".pdf,.xlsx,.xls,.xlsm,.csv,.tsv,.txt" hidden
                onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
              {file ? (
                <div>
                  <Ic d={I.check} size={24} color={C.green} />
                  <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginTop: 6 }}>{file.name}</div>
                  <div style={{ fontSize: 10, color: C.textMuted, marginTop: 2 }}>{(file.size / 1024).toFixed(0)} KB — Click to change</div>
                </div>
              ) : (
                <div>
                  <Ic d={I.upload} size={28} color={C.textDim} />
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.text, marginTop: 8 }}>Drop proposal here or click to browse</div>
                  <div style={{ fontSize: 10, color: C.textDim, marginTop: 4 }}>Supports PDF, Excel (.xlsx/.xls), CSV</div>
                </div>
              )}
            </div>

            {error && <div style={{ fontSize: 11, color: C.red, padding: "6px 10px", background: `${C.red}12`, borderRadius: 6 }}>{error}</div>}

            <button className="accent-btn" onClick={processFile} disabled={!file}
              style={bt(C, {
                background: file ? `linear-gradient(135deg, ${C.accent}, ${C.purple})` : C.bg3,
                color: file ? "#fff" : C.textDim, padding: "10px 20px", fontSize: 12,
                fontWeight: 600, border: "none", width: "100%",
        ...
        {step === "processing" && (
          <div style={{ padding: 48, textAlign: "center" }}>
            <div style={{ width: 48, height: 48, borderRadius: 24, border: `3px solid ${C.border}`, borderTopColor: C.accent, animation: "spin 0.8s linear infinite", margin: "0 auto" }} />
            <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginTop: 16 }}>Analyzing proposal...</div>
            <div style={{ fontSize: 11, color: C.textMuted, marginTop: 4 }}>AI is extracting line items, specifications, trade, and project location from {file?.name}</div>
          </div>
        )}
        ...
        {step === "review" && (
          <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
            {/* Review toolbar */}
            <div style={{ padding: "10px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <button onClick={toggleAll} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 4, padding: "4px 10px", fontSize: 10, fontWeight: 600, color: C.textMuted, cursor: "pointer" }}>
                {selectedIds.size === extractedItems.length ? "Deselect All" : "Select All"}
              </button>
              <span style={{ fontSize: 11, color: C.textMuted }}>{selectedIds.size} of {extractedItems.length} items selected</span>
              {subName && <span style={{ fontSize: 10, fontWeight: 600, color: C.accent, background: C.accentBg, padding: "2px 8px", borderRadius: 4 }}>{subName}</span>}
              {trade && <span style={{ fontSize: 10, fontWeight: 600, color: C.purple, background: `${C.purple}15`, padding: "2px 8px", borderRadius: 4 }}>{TRADE_OPTIONS.find(t => t.key === trade)?.label || trade}</span>}
              <div style={{ flex: 1 }} />
              <button onClick={() => { setStep("upload"); setExtractedItems([]); setRawItems([]); }}
                style={bt(C, { background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, padding: "5px 12px", fontSize: 10 })}>
                <Ic d={I.refresh} size={10} color={C.textMuted} /> Re-upload
              </button>
            </div>

            {/* Location Normalization Bar */}
            <div style={{ padding: "8px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10, background: hasLocationData && normalize ? `${C.green}06` : "transparent" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input type="checkbox" checked={normalize && hasLocationData} disabled={!hasLocationData}
            ...
            {/* Table headers */}
            <div style={{ display: "grid", gridTemplateColumns: "28px 68px 1.2fr 1fr 100px 44px 58px 58px 58px 58px", gap: 4, padding: "8px 16px", borderBottom: `1px solid ${C.border}`, fontSize: 8, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8 }}>
              <div></div><div>Code</div><div>Item</div><div>Specification</div><div>Trade</div><div>Unit</div>
              <div style={{ textAlign: "right" }}>Matl</div><div style={{ textAlign: "right" }}>Labor</div><div style={{ textAlign: "right" }}>Equip</div>
              <div style={{ textAlign: "right" }}>Sub Rate{extractedItems[0]?._normalized && <div style={{ fontSize: 6, color: C.textDim, fontWeight: 400 }}>not adjusted</div>}</div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "0 16px" }}>
              {extractedItems.map((item, idx) => {
                ...
                return (
                  <div key={item.id} style={{ display: "grid", gridTemplateColumns: "28px 68px 1.2fr 1fr 100px 44px 58px 58px 58px 58px", gap: 4, padding: "5px 0", borderBottom: `1px solid ${C.bg2}`, alignItems: "center", opacity: isSelected ? 1 : 0.4 }}>
                    <div>
                    ...
                    <input value={item.specText} onChange={e => updateExtractedItem(item.id, "specText", e.target.value)}
                      style={inp(C, { fontSize: 9, padding: "3px 5px", color: C.textMuted })}
                      placeholder="Material/performance spec..." />
                    <select value={item.trade} onChange={e => updateExtractedItem(item.id, "trade", e.target.value)}
                      style={inp(C, { fontSize: 9, padding: "3px 2px" })}>
                      <option value="">—</option>
                      {TRADE_OPTIONS.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}
                    </select>
                    <select value={item.unit} onChange={e => updateExtractedItem(item.id, "unit", e.target.value)}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                      <input type="number" value={item.material} onChange={e => updateExtractedItem(item.id, "material", parseFloat(e.target.value) || 0)}
                      ...
                      {item._normalized && item._originalMaterial > 0 && item._originalMaterial !== item.material && (
                        <div style={{ fontSize: 7, color: C.textDim, textDecoration: "line-through", marginTop: 1 }}>{item._originalMaterial.toFixed(2)}</div>
                      )}
                    </div>
                    {/* Labor — with original value annotation */}
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                      <input type="number" value={item.labor} onChange={e => updateExtractedItem(item.id, "labor", parseFloat(e.target.value) || 0)}
                      ...
                      {item._normalized && item._originalLabor > 0 && item._originalLabor !== item.labor && (
                        <div style={{ fontSize: 7, color: C.textDim, textDecoration: "line-through", marginTop: 1 }}>{item._originalLabor.toFixed(2)}</div>
                      )}
                    </div>
                    {/* Equipment — with original value annotation */}
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                      <input type="number" value={item.equipment} onChange={e => updateExtractedItem(item.id, "equipment", parseFloat(e.target.value) || 0)}
                        style={nInp(C, { fontSize: 10, padding: "3px 3px", color: C.orange })} />
                      {item._normalized && item._originalEquipment > 0 && item._originalEquipment !== item.equipment && (
                        <div style={{ fontSize: 7, color: C.textDim, textDecoration: "line-through", marginTop: 1 }}>{item._originalEquipment.toFixed(2)}</div>
                      )}
                    </div>
                    {/* Subcontractor — never normalized */}
                    <input type="number" value={item.subcontractor} onChange={e => updateExtractedItem(item.id, "subcontractor", parseFloat(e.target.value) || 0)}
            ...
            <div style={{ padding: "12px 20px", borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={onClose}
                  style={bt(C, { background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, padding: "8px 16px", fontSize: 11 })}>
                  Cancel
                </button>
                <button className="accent-btn" onClick={importSelected} disabled={selectedIds.size === 0}
                  style={bt(C, {
                    background: selectedIds.size > 0 ? C.accent : C.bg3,
                    color: selectedIds.size > 0 ? "#fff" : C.textDim,
                    padding: "8px 20px", fontSize: 11, fontWeight: 600, border: "none",
                  })}>
