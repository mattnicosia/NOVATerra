      ...
      {[
        ...
        <button
          ...
          style={{
            ...
            color: item.danger ? C.red : C.text, transition: 'background 0.15s',
          }}
...
function ConfirmDelete({ name, onConfirm, onCancel }) {
  ...
  return (
    <div style={{
      ...
      <div ref={ref} style={{
        background: C.sidebarBg, border: `1px solid ${C.border}`,
        borderRadius: 14, padding: '24px 28px', maxWidth: 340, width: '90%',
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button onClick={onConfirm} style={{
            padding: '7px 16px', borderRadius: 7, border: `1px solid ${C.red}4D`,
            background: `${C.red}26`, color: C.red, fontSize: 11, fontWeight: 600, cursor: 'pointer',
          }}>Delete</button>
        </div>
      </div>
    </div>
...
export default function ProjectsWidget() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: font }}>
      ...
      <div style={{ marginBottom: 8, padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{
      ...
      <div style={{ ...glassCardStyle, overflow: 'auto', flex: 1, minHeight: 0 }}>
        {hasEstimates ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  {(isHovered || menuState?.id === proj.id) && (
                    <button
                      ...
                      style={{
                        width: 20, height: 20, borderRadius: 4, border: 'none', background: ov(0.06),
                        cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
                        padding: 0, flexShrink: 0, marginLeft: 4, transition: 'background 0.15s',
                      }}
                      onMouseEnter={(e) => e.currentTarget.style.background = ov(0.12)}
                      onMouseLeave={(e) => e.currentTarget.style.background = ov(0.06)}
                    ><DotsIcon color={C.textMuted} /></button>
                  )}
                  {!(isHovered || menuState?.id === proj.id) && (
                    <span style={{
                      fontSize: 8, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase',
                      fontFamily: font, padding: '2px 6px', borderRadius: 4, flexShrink: 0, marginLeft: 6,
                      color: C.textMuted, background: ov(0.05), border: `1px solid ${C.borderLight}`,
                    }}>{proj.statusLabel}</span>
                  )}
                </div>
          ...
          <div style={{ padding: '20px 12px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 9.5, fontWeight: 400, color: C.textDim, fontFamily: font, marginTop: 4 }}>Create your first estimate below</div>
          </div>
        )}
      </div>
      ...
      {menuState && (
        <ProjectMenu x={menuState.x} y={menuState.y}
