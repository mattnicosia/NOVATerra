...
function resolveType(bt) {
  ...
  if (s.includes("residential") || s.includes("apartment") || s.includes("condo") || s.includes("housing"))
  ...
  const rows = [
    {
      ...
      fill: hasData ? Math.min(1, kgCO2ePerSF / (benchmark.high * 1.5)) : 0,
      color: sColor,
    },
    {
      label: "Total CO2e",
    ...
    {
      label: "LEED Target",
      value: hasData ? `${kgCO2ePerSF <= benchmark.typical * 0.8 ? "\u2713" : "\u2717"} -20%` : "\u2014",
      sub: hasData ? `need \u2264${(benchmark.typical * 0.8).toFixed(0)} kg/SF` : "",
      fill: hasData ? Math.min(1, Math.max(0, 1 - kgCO2ePerSF / (benchmark.typical * 0.8))) : 0,
        ...
        >
          <div
            style={{
              fontSize: 18,
              fontWeight: 700,
              color: hasData ? sColor : C.textDim,
