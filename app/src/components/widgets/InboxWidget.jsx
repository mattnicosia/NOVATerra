...
export default function InboxWidget() {
  ...
  // Fetch on mount
  useEffect(() => { loadReadIds(); fetchRfps(); }, []);

  const pendingRfps = rfps.filter(r => r.status === 'parsed' || r.status === 'pending');
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {!loading && pendingRfps.length === 0 && (
          <div style={{
            ...
            <div style={{ fontSize: 10, color: C.textDim, textAlign: 'center', lineHeight: 1.5 }}>
              No pending RFPs
            </div>
            <div style={{ fontSize: 9, color: C.textDim, textAlign: 'center', opacity: 0.6 }}>
              Forward bid invitations to<br />
        ...
        {pendingRfps.slice(0, 4).map(rfp => (
          ...
          >
            ...
            <div style={{
              ...
              <span style={{ fontSize: 9, color: C.textDim, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>
