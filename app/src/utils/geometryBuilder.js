...
const ARCH_MAP = { full: 1, half: 0.5, "3-8": 3/8, quarter: 1/4, "3-16": 3/16, eighth: 1/8, "3-32": 3/32, sixteenth: 1/16 };
...
function pxToFeet(drawingId, pxDist) {
...
function pointToFeet(pt, drawingId) {
  const ppf = getPxPerFoot(drawingId);
  if (!ppf) return null;
...
// ── Trade color palette ──────────────────────────────────────────
const TRADE_COLORS = {
  general: '#6B7280', demo: '#EF4444', sitework: '#92400E', concrete: '#9CA3AF',
  masonry: '#B45309', steel: '#3B82F6', framing: '#F59E0B', finishCarp: '#D97706',
  insulation: '#EC4899', roofing: '#7C3AED', doors: '#10B981', windows: '#06B6D4',
  drywall: '#F3F4F6', tile: '#8B5CF6', act: '#A78BFA', flooring: '#84CC16',
  painting: '#FB923C', specialties: '#14B8A6', elevator: '#6366F1',
  fireSuppression: '#DC2626', plumbing: '#2563EB', hvac: '#059669', electrical: '#EAB308',
  unassigned: '#6B7280',
};
...
function getSpecValue(takeoff, specId) {
  ...
  for (const [modId, inst] of Object.entries(moduleState.moduleInstances || {})) {
    ...
    for (const [catId, instances] of Object.entries(inst.categoryInstances || {})) {
