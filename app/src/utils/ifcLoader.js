...
const EXTRACT_TYPES = [
  ...
  WebIFC.IFCSLAB, WebIFC.IFCDOOR, WebIFC.IFCWINDOW,
  WebIFC.IFCCOLUMN, WebIFC.IFCBEAM, WebIFC.IFCROOF,
  WebIFC.IFCSTAIR, WebIFC.IFCSTAIRFLIGHT, WebIFC.IFCRAILING,
  WebIFC.IFCCURTAINWALL, WebIFC.IFCPLATE, WebIFC.IFCMEMBER,
  ...
  try {
    ...
    for (let i = 0; i < relIds.size(); i++) {
      try {
        ...
        let storeyName = '', elevation = 0, storeyId = '';
        try {
          ...
          if (Array.isArray(pset.Quantities)) {
            for (const qr of pset.Quantities) {
              ...
              try {
                ...
                const qval = q.AreaValue?.value ?? q.VolumeValue?.value ?? q.LengthValue?.value ?? q.CountValue?.value ?? q.WeightValue?.value;
