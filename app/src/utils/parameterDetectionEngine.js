...
const SOURCE_WEIGHTS = {
  geometry: 1.0, // Rooms are physically enclosed — geometry can't hallucinate walls
  ...
  "ai-vision": 0.7, // Full-sheet 2000px analysis — decent but misses small fixtures
  "door-inference": 0.5, // Heuristic from door widths — supplementary signal
};
...
export async function runParameterDetection({ parsedSchedules = [], scheduleEvidence = null, onProgress } = {}) {
  ...
  console.log(
    `[DetectionEngine] Complete in ${timing.total}ms — ${allEvidence.length} evidence items, ${Object.keys(parameters).length} parameters detected`,
  );
...
async function detectRoomsFromGeometry(floorPlanDrawings) {
  ...
  for (const drawing of floorPlanDrawings) {
    ...
    try {
      ...
      console.log(
        `[DetectionEngine:GeoA] ${drawing.sheetTitle || drawing.id}: found ${geoResult.rooms.length} rooms, ${geoResult.roomLabels?.length || 0} labels`,
      );
        ...
        if (parsed.rooms) {
          ...
          for (const [aiKey, count] of Object.entries(parsed.rooms)) {
            ...
            evidence.push({
              ...
              value: count,
              confidence: parsed.roomConfidence?.[aiKey] || 0.7,
...
function summarizeEvidence(evidence) {
  ...
  for (const [param, items] of Object.entries(byParam)) {
    const best = items.reduce((a, b) =>
      a.confidence * (SOURCE_WEIGHTS[a.source] || 0.5) > b.confidence * (SOURCE_WEIGHTS[b.source] || 0.5) ? a : b,
    );
...
export function mergeEvidence(allEvidence, correctionFactors = {}) {
  ...
  for (const [paramPath, items] of Object.entries(byParam)) {
    ...
    const isTargetedRoomCount = paramPath.startsWith("roomCounts.") && items.some(e => e.source === "ai-targeted");

    if (isTargetedRoomCount) {
      // Sum targeted counts, then compare with other sources
      const targetedSum = items.filter(e => e.source === "ai-targeted").reduce((s, e) => s + e.value, 0);
      const otherItems = items.filter(e => e.source !== "ai-targeted");

      // Create a synthetic evidence entry for aggregated targeted count
      if (targetedSum > 0) {
        const avgConf =
          items.filter(e => e.source === "ai-targeted").reduce((s, e) => s + e.confidence, 0) /
          items.filter(e => e.source === "ai-targeted").length;
        otherItems.push({
          source: "ai-targeted",
          paramPath,
          value: targetedSum,
...
function mergeParamValues(paramPath, items, correctionFactors) {
  ...
  if (isNumeric) {
    ...
    const agreementBoost = distinctValues === 1 ? 0.1 : -0.05 * (distinctValues - 1);
