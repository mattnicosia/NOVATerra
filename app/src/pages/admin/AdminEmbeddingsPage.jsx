...
export default function AdminEmbeddingsPage() {
  ...
  if (loading) {
    return <div style={{ color: C.textMuted, fontSize: 13, padding: 40, textAlign: "center" }}>Loading embeddings...</div>;
  }
  if (error) {
      ...
      {byUser.length > 0 && (
        <div style={{ ...card(C), overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <span>User</span>
            {kinds.map(k => (
              <span key={k} style={{ textAlign: "right" }}>{k.replace("_", " ")}</span>
            ))}
            <span style={{ textAlign: "right" }}>Total</span>
          </div>

          {/* Rows */}
          {byUser.sort((a, b) => b.total - a.total).map(row => (
