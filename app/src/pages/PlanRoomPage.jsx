...
export default function PlanRoomPage() {
  ...
  const activeLoc = manualMetro ? { mat: manualMetro.mat, lab: manualMetro.lab, equip: manualMetro.equip, label: manualMetro.label, source: "metro" } : locRaw;
  ...
  const hasData = drawings.length > 0 || specs.length > 0 || floors.length > 0 || filledRooms.length > 0 || scanResults || Object.keys(autoDetected).length > 0;
  ...
  const handleApplyToEstimate = (selectedItems) => {
    ...
    selectedItems.forEach(li => {
      ...
      addElement(division, { code: li.code, name: li.description, unit: li.unit, material: li.m || 0, labor: li.l || 0, equipment: li.e || 0, quantity: li.qty || 1 });
      count++;
    });
  ...
  if (isSetupMode) {
    ...
    return (
      <div style={{
        ...
        minHeight: "100%", padding: T.space[7], fontFamily: "'DM Sans', sans-serif",
      }}>
        <div style={{ maxWidth: 520, width: "100%", textAlign: "center" }}>
          <div style={{ marginBottom: 24, display: "flex", justifyContent: "center" }}>
            <NovaOrb size={48} scheme="nova" />
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, margin: 0, marginBottom: 6, fontFamily: "'DM Sans', sans-serif" }}>
            Upload Your Construction Plans
          </h1>
          <p style={{ fontSize: 12, color: C.textDim, margin: 0, marginBottom: 28, lineHeight: 1.6 }}>
            Drop your PDF plans below and NOVA will automatically extract project details,
            detect schedules, and generate a rough order of magnitude estimate.
          </p>
          {isProcessing ? (
            <div style={{ ...card(C), padding: "24px 28px", marginBottom: 20, textAlign: "left", border: `1px solid ${C.accent}20` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>
                  NOVA is analyzing your drawings...
                </div>
              </div>
              {scanProgress.phase && (
          ...
          {!isProcessing && (
            <button onClick={handleSkip} style={{
              ...
              fontSize: 11, color: C.textDim, padding: "8px 16px",
    ...
    <div style={{ padding: T.space[7], minHeight: "100%" }}>
      <div style={{ maxWidth: 1000 }}>
        ...
        <div style={{ marginBottom: T.space[5], display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <h1 style={{ fontSize: T.fontSize.xl, fontWeight: T.fontWeight.bold, color: C.text, margin: 0, display: "flex", alignItems: "center", gap: T.space[3], fontFamily: "'DM Sans',sans-serif" }}>
              <Ic d={I.plans} size={22} color={C.accent} />
              Discovery
              {documents.length > 0 && (
                <span style={{ fontSize: T.fontSize.xs, color: C.textDim, padding: "2px 8px", borderRadius: T.radius.full, background: C.bg2, fontWeight: T.fontWeight.medium }}>
                  {documents.length} file{documents.length !== 1 ? "s" : ""}
                </span>
              )}
            </h1>
            <p style={{ fontSize: T.fontSize.xs, color: C.textDim, margin: `${T.space[1]}px 0 0`, fontFamily: "'DM Sans',sans-serif" }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {hasData && (
              <button
                ...
                style={bt(C, {
                  fontSize: 10, padding: "5px 12px", borderRadius: T.radius.sm,
                  color: C.red || "#ef4444",
        ...
        {documents.length === 0 || uploadExpanded ? (
          <div
            ...
            onClick={() => fileInputRef.current?.click()}
            style={{
              ...card(C), padding: documents.length === 0 ? T.space[6] : T.space[4],
              textAlign: "center", cursor: "pointer",
              border: dragOver ? `2px dashed ${C.accent}` : `2px dashed ${C.border}`,
              background: dragOver ? `${C.accent}08` : C.glassBg,
              transition: "all 0.2s ease", marginBottom: T.space[4],
            }}
          >
            {documents.length === 0 ? (
              <>
                ...
                <div style={{ fontSize: T.fontSize.sm, fontWeight: T.fontWeight.semibold, color: C.text, marginBottom: T.space[1] }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}>
                ...
                <button
                  onClick={e => { e.stopPropagation(); setUploadExpanded(false); }}
                  style={{ background: "transparent", border: "none", cursor: "pointer", padding: "2px 6px", color: C.textDim, fontSize: 10 }}
                >Cancel</button>
              </div>
            )}
            <input ref={fileInputRef} type="file" multiple
          ...
          <button
            ...
            style={bt(C, {
              ...
              padding: "6px 14px", marginBottom: T.space[4],
              background: `${C.accent}06`, border: `1px solid ${C.accent}20`,
              borderRadius: T.radius.sm, cursor: "pointer",
              transition: "all 0.15s ease",
            })}
            onMouseEnter={e => { e.currentTarget.style.background = `${C.accent}12`; e.currentTarget.style.borderColor = `${C.accent}40`; }}
            onMouseLeave={e => { e.currentTarget.style.background = `${C.accent}06`; e.currentTarget.style.borderColor = `${C.accent}20`; }}
          >
        ...
        {documents.length > 0 && (
          <CompactDocList
            drawingDocs={drawingDocs} specDocs={specDocs} generalDocs={generalDocs}
            onRemove={removeDocument} C={C} T={T}
          />
        )}
        ...
        {showResetConfirm && (
          <div style={{
            ...
            background: `${C.red || "#ef4444"}06`, borderRadius: T.radius.md,
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 10, color: C.textDim, marginTop: 2 }}>
                This will clear all drawings, specs, scan results, terrain parameters, and model data. Project info (name, client, dates) will be preserved.
              </div>
            </div>
            <button onClick={() => setShowResetConfirm(false)}
              style={bt(C, { fontSize: 10, padding: "5px 14px", background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted })}>
              Cancel
            </button>
            <button onClick={handleResetAll}
              style={bt(C, { fontSize: 10, padding: "5px 14px", fontWeight: 600, background: C.red || "#ef4444", color: "#fff" })}>
              Reset
            </button>
          </div>
        )}
        ...
        {processingDocs.length > 0 && (
          <div style={{
            marginBottom: T.space[4], padding: `${T.space[3]}px ${T.space[4]}px`,
            background: `${C.accent}06`, borderRadius: T.radius.md, border: `1px solid ${C.accent}20`,
        ...
        {hasData && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: T.space[4] }}>
              ...
              <div style={{ ...card(C), padding: T.space[5], gridColumn: "1 / -1" }}>
                <div style={{ display: "flex", alignItems: "center", gap: T.space[2], marginBottom: T.space[3] }}>
                  ...
                  <button
                    ...
                    style={bt(C, {
                      fontSize: 10, padding: "4px 10px", borderRadius: T.radius.sm,
                  ...
                  >
                    {rescanning ? (
                      <>
                        <span style={{
                          display: "inline-block", width: 10, height: 10,
                          border: `2px solid ${C.textDim}40`, borderTop: `2px solid ${C.textDim}`,
                      ...
                      <>
                        <Ic d={I.refresh} size={11} color={C.orange || "#F59E0B"} />
                        {" "}Rescan
                      </>
                    )}
                  </button>
                  <button onClick={() => navigate(`/estimate/${estId}/insights`)}
                    style={bt(C, { fontSize: 10, color: C.accent, padding: "4px 10px", background: `${C.accent}08`, border: `1px solid ${C.accent}20`, borderRadius: T.radius.sm })}>
                ...
                {rescanning && scanProgress?.phase && (
                  <div style={{
                    ...
                    padding: "8px 14px", marginBottom: T.space[3],
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        width: scanProgress.total > 0 ? `${Math.round((scanProgress.current / scanProgress.total) * 100)}%` : "30%",
                ...
                <div style={{ display: "flex", gap: T.space[3], flexWrap: "wrap" }}>
                  ...
                  <PipelineStep
                    ...
                    detail={scanResults?.drawingNotes ? `${scanResults.drawingNotes.reduce((s, r) => s + (r.notes?.length || 0), 0)} notes` : null}
                    C={C} T={T}
                  />
                  <PipelineStep
            ...
            <div style={{ ...card(C), padding: T.space[5], gridColumn: "1 / -1" }}>
              <div style={{ display: "flex", alignItems: "center", gap: T.space[2], marginBottom: T.space[3] }}>
                ...
                {Object.keys(autoDetected).length > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 600, color: C.green, background: `${C.green}12`, padding: "2px 8px", borderRadius: T.radius.full, display: "flex", alignItems: "center", gap: 3 }}>
              ...
              {(project.name && project.name !== "New Estimate") || project.architect || project.client || project.address || project.projectNumber || project.engineer ? (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: `${T.space[2]}px ${T.space[4]}px`, marginBottom: T.space[3], padding: `${T.space[3]}px`, background: `${C.accent}04`, borderRadius: T.radius.sm, border: `1px solid ${C.border}08` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: activeLoc && activeLoc.source !== "none" ? "1fr 1fr" : "1fr", gap: T.space[4] }}>
                {/* Left: stat pills */}
                ...
                {activeLoc && activeLoc.source !== "none" && (
                  <div style={{ padding: `${T.space[3]}px`, background: `${C.text}03`, borderRadius: T.radius.sm, border: `1px solid ${C.border}08` }}>
                    {/* Metro badge */}
                    <div style={{
                    ...
                    {/* Cost factor bars */}
                    <div style={{ fontSize: 9, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: T.space[2] }}>
            ...
            {/* ─── NOVA ROM Card ─── */}
            {scanResults && (
              <div style={{ ...card(C), padding: T.space[5], gridColumn: "1 / -1", border: `1px solid ${C.purple || C.accent}15` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: T.space[3] }}>
                  <div style={{ display: "flex", alignItems: "center", gap: T.space[2] }}>
                    <Ic d={I.ai} size={16} color={C.purple || C.accent} />
                    <span style={{ fontSize: T.fontSize.sm, fontWeight: T.fontWeight.bold, color: C.purple || C.accent }}>NOVA ROM</span>
                  </div>
                  {scanResults.rom?.sfEstimated && (
                    <span style={{ fontSize: 9, color: C.orange, fontWeight: 500, background: `${C.orange}10`, padding: "2px 8px", borderRadius: T.radius.full }}>
                      SF estimated by AI
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: T.space[4] }}>
                  {scanResults.rom?.projectSF && scanResults.rom?.totals && (
                    <StatPill label="Cost/SF" value={`$${Math.round(scanResults.rom.totals.mid / scanResults.rom.projectSF)}`} unit="/SF" C={C} T={T} accent />
                  )}
                  ...
                  {scanResults.drawingNotes && (
                    <StatPill label="Drawing Notes" value={scanResults.drawingNotes.reduce((s, r) => s + (r.notes?.length || 0), 0)} unit="extracted" C={C} T={T} />
                  )}
                </div>
                  ...
                  <div style={{ marginTop: T.space[3], display: "flex", flexWrap: "wrap", gap: T.space[2] }}>
                    {scanResults.schedules.map((s, i) => (
                      <span key={i} style={{
                        ...
                        background: `${C.purple || C.accent}08`, fontSize: 10,
                        color: C.purple || C.accent, fontWeight: 500,
                      }}>
              ...
              <div style={{ ...card(C), padding: T.space[5] }}>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: T.space[2] }}>
                    ...
                    <span style={{ fontSize: T.fontSize.sm, fontWeight: T.fontWeight.bold, color: C.text }}>Drawing Index</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: T.space[2] }}>
                    <span style={{ fontSize: 10, color: labeledCount === drawings.length ? C.green : C.textMuted, fontWeight: 600 }}>{labeledCount}/{drawings.length} labeled</span>
                    <span style={{ fontSize: 10, color: scaledCount === drawings.length ? C.green : C.textMuted, fontWeight: 600 }}>{scaledCount}/{drawings.length} scaled</span>
                    <Ic d={I.chevron} size={10} color={C.textDim} style={{ transform: drawingsExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s" }} />
                  </div>
                </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: T.space[2], marginBottom: T.space[2] }}>
                    {Object.entries(disciplines).sort((a, b) => b[1] - a[1]).map(([name, count]) => (
                      <span key={name} style={{
                        ...
                        background: `${C.blue}08`, fontSize: 10, fontWeight: 500, color: C.blue,
                      }}>
                        {name}: {count}
                      </span>
                    ))}
                ...
                <div style={{ maxHeight: drawingsExpanded ? "none" : 200, overflowY: drawingsExpanded ? "visible" : "auto", transition: "max-height 0.3s ease" }}>
                  {drawings.map(d => (
                    <div key={d.id} style={{ display: "flex", alignItems: "center", gap: T.space[2], padding: "4px 0", borderBottom: `1px solid ${C.border}08`, fontSize: T.fontSize.xs }}>
                      {/* Thumbnail */}
                      <div style={{ width: 32, height: 22, borderRadius: 2, overflow: "hidden", background: C.bg2, flexShrink: 0 }}>
                      ...
                      {outlines[d.id] && (
                        <span style={{ fontSize: 8, fontWeight: 600, color: C.green, background: `${C.green}12`, padding: "1px 5px", borderRadius: 3 }}>OUTLINE</span>
                      )}
                      {drawingScales[d.id] && (
                ...
                {!drawingsExpanded && drawings.length > 8 && (
                  <div
                    ...
                    style={{ textAlign: "center", padding: "6px 0", fontSize: 10, color: C.accent, cursor: "pointer", fontWeight: 600, marginTop: 2 }}
                  >
            ...
            {specs.length > 0 && (
              <div style={{ ...card(C), padding: T.space[5] }}>
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: T.space[2] }}>
                    <span style={{ fontSize: 10, color: allocatedSpecs.length === specs.length ? C.green : C.orange, fontWeight: 600 }}>
                      {allocatedSpecs.length}/{specs.length} allocated
                    </span>
                    <Ic d={I.chevron} size={10} color={C.textDim} style={{ transform: specsExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s" }} />
                  </div>
                </div>
                <div style={{ maxHeight: specsExpanded ? "none" : 200, overflowY: specsExpanded ? "visible" : "auto", transition: "max-height 0.3s ease" }}>
                  {specs.map(sp => {
                    ...
                    return (
                      <div key={sp.id} style={{ display: "flex", alignItems: "center", gap: T.space[2], padding: "4px 0", borderBottom: `1px solid ${C.border}08`, fontSize: T.fontSize.xs }}>
                        ...
                        {allocated ? (
                          <span style={{ fontSize: 8, fontWeight: 700, color: C.green, background: `${C.green}12`, padding: "2px 6px", borderRadius: 3 }}>ALLOC</span>
                        ) : (
                          <span style={{ fontSize: 8, fontWeight: 600, color: C.orange, background: `${C.orange}12`, padding: "2px 6px", borderRadius: 3 }}>GAP</span>
                        )}
                      </div>
                    );
                  })}
                ...
                {specs.length > 0 && allocatedSpecs.length < specs.length && (
                  <div style={{ marginTop: T.space[2], fontSize: 10, color: C.orange, fontWeight: 500 }}>
                    {specs.length - allocatedSpecs.length} unallocated spec section{specs.length - allocatedSpecs.length > 1 ? "s" : ""} — add scope items to cover
                  </div>
                )}
            ...
            {novaHistory.length > 0 && (
              <div style={{ ...card(C), padding: T.space[5], gridColumn: "1 / -1" }}>
                <div style={{ display: "flex", alignItems: "center", gap: T.space[2], marginBottom: T.space[3] }}>
                  ...
                  <span style={{ fontSize: T.fontSize.sm, fontWeight: T.fontWeight.bold, color: C.textMuted }}>NOVA Activity Log</span>
                </div>
          ...
          <div style={{ marginTop: T.space[5], ...card(C), padding: T.space[5] }}>
            ...
            <div style={{ display: "flex", gap: T.space[3], flexWrap: "wrap" }}>
              <button onClick={() => navigate(`/estimate/${estId}/takeoffs`)}
                ...
                <span style={{ display: "block", fontSize: 9, fontWeight: 400, opacity: 0.8, marginTop: 2 }}>Measure quantities from drawings</span>
              </button>
              ...
              <button onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); setUploadExpanded(true); }}
                ...
                <span style={{ display: "block", fontSize: 9, fontWeight: 400, opacity: 0.7, marginTop: 2 }}>Add drawings, specs, or addenda</span>
              </button>
            </div>
          </div>
        )}
        ...
        {scanResults && !scanProgress.phase && !showScanModal && (
          <div style={{ marginTop: T.space[4], ...card(C), padding: `${T.space[3]}px ${T.space[4]}px`, border: `1px solid ${C.purple || C.accent}20` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: C.purple || C.accent, marginBottom: 2, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {showScanModal && scanResults && (
        <ScanResultsModal
          ...
          onSaveOnly={() => { setShowScanModal(false); showToast("Scan results saved"); }}
        />
      )}
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: T.space[4], ...card(C), padding: 0, overflow: "hidden" }}>
      <div style={{ padding: `${T.space[2]}px ${T.space[3]}px`, display: "flex", alignItems: "center", gap: 6, borderBottom: `1px solid ${C.border}08` }}>
        ...
        {drawingDocs.length > 0 && <span style={{ fontSize: 9, color: C.blue, fontWeight: 500 }}>{drawingDocs.length} drawing{drawingDocs.length !== 1 ? "s" : ""}</span>}
        {specDocs.length > 0 && <span style={{ fontSize: 9, color: C.purple || C.accent, fontWeight: 500 }}>{specDocs.length} spec{specDocs.length !== 1 ? "s" : ""}</span>}
        {generalDocs.length > 0 && <span style={{ fontSize: 9, color: C.textDim, fontWeight: 500 }}>{generalDocs.length} other</span>}
      </div>
      {preview.map(doc => (
        <div key={doc.id} style={{
          display: "flex", alignItems: "center", gap: T.space[2],
          padding: `4px ${T.space[3]}px`, borderBottom: `1px solid ${C.border}06`,
          fontSize: 11,
        }}>
          <span style={{
            fontSize: 8, fontWeight: 700, color: doc.docType === "drawing" ? C.blue : doc.docType === "specification" ? (C.purple || C.accent) : C.textDim,
            textTransform: "uppercase", minWidth: 30,
          }}>
          ...
          <button onClick={() => onRemove(doc.id)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: 2, opacity: 0.4 }}
      ...
      {allDocs.length > 4 && !expanded && (
        <div onClick={() => setExpanded(true)} style={{ padding: "4px 0", textAlign: "center", fontSize: 10, color: C.accent, cursor: "pointer", fontWeight: 600 }}>
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      ...
      <div style={{ height: 6, background: C.bg2, borderRadius: 3, overflow: "hidden", position: "relative" }}>
        <div style={{ position: "absolute", left: `${natPct}%`, top: 0, width: 1, height: 6, background: `${C.textDim}40`, zIndex: 1 }} />
        <div style={{
...
function StatPill({ label, value, unit, C, T, accent }) {
  return (
    <div style={{ minWidth: 80 }}>
      <div style={{ fontSize: 9, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 600, marginBottom: 2 }}>
        {label}
      </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 1 }}>
        <span style={{ fontSize: 9, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 600 }}>{label}</span>
        {detected && (
          <span style={{
            fontSize: 7, fontWeight: 700, color: C.green, background: `${C.green}15`,
            padding: "1px 4px", borderRadius: 3, textTransform: "uppercase", letterSpacing: "0.03em",
          }}>AI</span>
        )}
      </div>
      <div style={{ fontSize: T.fontSize.xs, color: C.text, fontWeight: T.fontWeight.medium, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {value}
      </div>
    </div>
  );
...
function buildProjectNarrative(project, drawings, specs, activeLoc) {
  ...
  const feats = [];
  const roomLabels = {
    bathrooms: "bathroom", kitchens: "kitchen", bedrooms: "bedroom",
    offices: "office", conferenceRooms: "conference room", breakRooms: "break room",
    lobbies: "lobby", staircases: "staircase", elevators: "elevator",
    storageRooms: "storage room", parkingSpaces: "parking space",
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        {detail && <div style={{ fontSize: 8, color: done ? C.green : C.textDim, fontWeight: 500, marginTop: 1 }}>{detail}</div>}
      </div>
    </div>
  );
}
