    ...
    <div style={{ flex: 1, background: C.bg1, borderRadius: 8, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <span style={{ fontSize: 10, color: C.textDim, marginLeft: "auto" }}>
          {divCodes.length} divisions &middot; Baseline allocations
        </span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {divCodes.map((dc, idx) => {
          ...
          const divName = subs[0]?.label ? dc : dc;
          return (
            <div key={dc}>
              {/* Division header */}
              <div
              ...
              >
                ...
                <span style={{ fontSize: 10, color: C.textDim }}>
                  {subs.length} subdivision{subs.length !== 1 ? "s" : ""}
                </span>
              </div>
              ...
              {isExpanded && subs.length > 0 && (
                <div style={{ background: C.isDark ? "rgba(139,92,246,0.03)" : "rgba(139,92,246,0.015)", borderBottom: `1px solid ${C.border}` }}>
                  {/* Sub-header */}
                  <div style={{ display: "grid", gridTemplateColumns: "100px 1fr 80px 80px", gap: 4, padding: "4px 14px 4px 44px", fontSize: 8, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.6, borderBottom: `1px solid ${C.isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"}` }}>
                    <div>Code</div>
                    <div>Subdivision</div>
                  ...
                  {subs.map(sub => {
                    ...
                    const tier = getConfidenceTier(override ? "user" : "baseline");
                    const pct = override ? override.pctOfDiv : sub.pctOfDiv;
                    return (
                      <div key={sub.code} style={{ display: "grid", gridTemplateColumns: "100px 1fr 80px 80px", gap: 4, padding: "6px 14px 6px 44px", fontSize: 11, borderBottom: `1px solid ${C.isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)"}`, alignItems: "center" }}>
                        ...
                        <div style={{ textAlign: "right", fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: C.text, fontWeight: 500 }}>
                          {(pct * 100).toFixed(1)}%
                        </div>
                        <div style={{ textAlign: "center" }}>
                          <span style={{ fontSize: 8, fontWeight: 600, padding: "2px 6px", borderRadius: 4, background: override ? "rgba(34,197,94,0.12)" : "rgba(107,114,128,0.12)", color: override ? "#22C55E" : "#6B7280" }}>
                            {override ? "User" : "Baseline"}
                          </span>
                        </div>
                      </div>
                    );
      ...
      <div style={{ padding: "8px 14px", borderTop: `1px solid ${C.border}`, fontSize: 9, color: C.textDim, lineHeight: 1.6 }}>
        <strong>Baseline allocations</strong> are used as the default input in the confidence-weighted engine (60% weight).
        User overrides and LLM-generated refinements shift allocations when available.
      </div>
    </div>
  );
}
