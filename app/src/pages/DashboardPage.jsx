...
export default function DashboardPage() {
  ...
  const pipeline = companyEstimates.filter(e => e.status === "Bidding" || e.status === "Submitted").reduce((s, e) => s + nn(e.grandTotal), 0);
  ...
  const statusColor = (s) => {
    switch (s) {
      case "Bidding": return C.accent;
      case "Submitted": return C.orange;
      case "Won": return C.green;
      case "Lost": return C.red;
      case "On Hold": return C.purple;
      case "Cancelled": return C.textDim;
  ...
  if (showOnboarding) {
    const steps = [
      { icon: I.settings, label: "Company Profile", desc: "Add your company name, logo, and contact info for proposals.", done: step1Done, action: () => navigate("/settings"), color: C.accent },
      { icon: I.database, label: "Cost Database", desc: "Review pre-loaded cost items or add your own pricing data.", done: step2Done, action: () => navigate("/core?tab=database"), color: C.green },
      { icon: I.estimate, label: "First Estimate", desc: "Create your first project estimate and start building scope.", done: step3Done, action: () => handleCreate(), color: C.purple },
    ];
    return (
      <div style={{ ...pageContainer(C), display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", maxWidth: 680 }}>
          ...
          <div style={{ display: "flex", gap: T.space[4], justifyContent: "center" }}>
            {steps.map((s, i) => (
              <button key={i} onClick={s.action} style={{
                ...
                <div style={{ fontSize: T.fontSize.md, fontWeight: T.fontWeight.semibold, color: C.text, marginBottom: T.space[1] }}>
                  {s.label}
                </div>
          ...
          <button onClick={() => updateSetting("onboardingDismissed", true)} style={{
            background: "none", border: "none", cursor: "pointer",
            fontSize: T.fontSize.sm, color: C.textDim, marginTop: T.space[7],
  ...
  return (
    <div style={pageContainer(C)} className="blueprint-grid">
      ...
      {/* Page Title */}
      <h1 style={{
        fontSize: 24, fontWeight: T.fontWeight.heavy, color: C.text,
        letterSpacing: -0.5, margin: 0, marginBottom: T.space[6],
        lineHeight: T.lineHeight.tight,
      }}>Estimates</h1>

      {/* KPI Row */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: T.space[5], marginBottom: T.space[8] }}>
      ...
      <div style={{ ...card(C), overflow: "hidden", animation: "staggerFadeUp 450ms cubic-bezier(0.16,1,0.3,1) 500ms both" }}>
        {/* Table Header */}
        <div style={{ padding: `${T.space[4]}px ${T.space[5]}px`, borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={() => setCsvImportOpen(true)} style={bt(C, {
              background: "transparent", border: `1px solid ${C.border}`,
              color: C.textMuted, padding: "7px 14px", borderRadius: T.radius.sm,
            })}>
              <Ic d={I.upload} size={14} color={C.textMuted} /> Import CSV
            </button>
            <button onClick={handleCreate} className="accent-btn" style={bt(C, {
              background: C.gradient || C.accent, color: "#fff", padding: "7px 14px", borderRadius: T.radius.sm,
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: T.space[10], textAlign: "center" }}>
            ...
            <div style={{ fontSize: T.fontSize.lg, fontWeight: T.fontWeight.semibold, color: C.text, marginBottom: T.space[1] }}>
            ...
            <div style={{ fontSize: T.fontSize.base, color: C.textMuted, marginBottom: T.space[4] }}>
              {companyEstimates.length === 0 ? "Create your first estimate to get started." : "No estimates match your current filter."}
            </div>
            {companyEstimates.length === 0 && (
              <button onClick={handleCreate} className="accent-btn" style={bt(C, {
                background: C.gradient || C.accent, color: "#fff", padding: "10px 20px", borderRadius: T.radius.sm,
                boxShadow: `0 0 12px ${C.accent}20`,
              })}>
          ...
          <div>
            {/* Column headers */}
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 80px", padding: `${T.space[3]}px ${T.space[5]}px`, background: C.bg2, borderBottom: `1px solid ${C.border}`, ...sectionLabel(C) }}>
              <span>Name</span>
              <span>Client</span>
              <span>Status</span>
            ...
            {filtered.map((est, idx) => {
              ...
              >
                ...
                <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }} onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={() => setDeleteConfirm(est.id)} title="Delete" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, borderRadius: 4, display: "flex" }} className="icon-btn">
...
function InboxNotification({ C, T, navigate }) {
  ...
  return (
    <div
      style={{
        ...card(C), padding: `${T.space[3]}px ${T.space[4]}px`, marginBottom: T.space[5],
        display: "flex", alignItems: "center", gap: T.space[3], cursor: "pointer",
        background: `${C.accent}08`, border: `1px solid ${C.accent}25`,
      }}
      onClick={() => navigate("/inbox")}
      onMouseOver={e => { e.currentTarget.style.borderColor = C.accent + "50"; }}
      onMouseOut={e => { e.currentTarget.style.borderColor = C.accent + "25"; }}
    >
      <div style={{
      ...
      <span style={{ fontSize: T.fontSize.xs, color: C.accent, fontWeight: T.fontWeight.semibold }}>
        View Inbox →
      </span>
    </div>
  );
