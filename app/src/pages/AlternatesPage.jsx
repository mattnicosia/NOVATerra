...
export default function AlternatesPage() {
  ...
  const runVE = async () => {
    setVeLoading(true); setVeSuggestions([]);
    try {
      const specSummary = specs.map(s => `${s.section} ${s.title}: ${s.summary}`).join("\n");
      const itemSummary = items.slice(0, 60).map(i => `${i.code || ""} ${i.description} (${i.quantity} ${i.unit}) M:$${i.material} L:$${i.labor} E:$${i.equipment} S:$${i.subcontractor}`).join("\n");
    ...
    } catch (e) { showToast("VE analysis failed", "error"); console.error(e); }
    setVeLoading(false);
  };
    ...
    <div style={{ padding: T.space[7], minHeight: "100%" }}>
      <div style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          <button className="accent-btn" onClick={() => addAlternate("deduct")} style={bt(C, { background: C.green, color: "#fff", padding: "8px 16px" })}>
            <Ic d={I.plus} size={13} color="#fff" sw={2} /> Deduct Alternate
          </button>
          <button className="accent-btn" onClick={() => addAlternate("add")} style={bt(C, { background: C.blue, color: "#fff", padding: "8px 16px" })}>
            <Ic d={I.plus} size={13} color="#fff" sw={2} /> Add Alternate
          </button>
          <div style={{ flex: 1 }} />
          <button className="accent-btn" disabled={veLoading || (items.length === 0 && specs.length === 0)} onClick={runVE}
            style={bt(C, { background: C.purple, color: "#fff", padding: "8px 16px", opacity: (veLoading || (items.length === 0 && specs.length === 0)) ? 0.5 : 1 })}>
        ...
        {veSuggestions.length > 0 && (
          <div style={{ marginBottom: 16, padding: "12px 16px", background: "rgba(160,120,255,0.04)", borderRadius: 8, border: `1px solid ${C.purple}40` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.purple, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10 }}>AI Value Engineering Suggestions</div>
            {veSuggestions.map(s => (
              <div key={s.id} style={{ padding: "8px 12px", marginBottom: 6, background: s.accepted ? "rgba(76,175,125,0.06)" : C.bg1, borderRadius: 6, border: `1px solid ${s.accepted ? C.green + "40" : C.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: C.textDim, marginBottom: 2 }}>Currently: <strong style={{ color: C.text }}>{s.original}</strong></div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: C.green, marginBottom: 2 }}>{"\u2192"} {s.suggestion}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    }} disabled={s.accepted} style={bt(C, { fontSize: 10, padding: "4px 10px", background: s.accepted ? C.bg2 : C.green, color: s.accepted ? C.textDim : "#fff", border: "none" })}>
        ...
        {/* Alternates List */}
        {alternates.length === 0 ? (
          <div style={{ textAlign: "center", padding: T.space[10], border: `1px dashed ${C.border}`, borderRadius: T.radius.md }}>
            <div style={{
            ...
            <div style={{ fontSize: T.fontSize.lg, fontWeight: T.fontWeight.semibold, color: C.text, marginBottom: T.space[1] }}>No alternates yet</div>
            <div style={{ fontSize: T.fontSize.base, color: C.textMuted, maxWidth: 320, margin: "0 auto" }}>Add deduct alternates to offer cost savings, or add alternates for scope upgrades.</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {alternates.map((alt, altIdx) => {
              ...
              const { grand: altGrand, markupAmount } = getAltTotalWithMarkup(alt);
              return (
                <div key={alt.id} style={{ border: `1px solid ${alt.accepted ? "rgba(76,175,125,0.4)" : C.border}`, borderLeft: `3px solid ${alt.type === "deduct" ? C.green : C.blue}`, borderRadius: T.radius.md, overflow: "hidden", background: alt.accepted ? "rgba(76,175,125,0.03)" : "transparent" }}>
                  {/* Header */}
                  <div style={{ padding: "10px 14px", background: C.bg1, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 4, color: alt.type === "deduct" ? C.green : C.blue, background: alt.type === "deduct" ? "rgba(76,175,125,0.12)" : "rgba(91,141,239,0.12)", border: `1px solid ${alt.type === "deduct" ? C.green : C.blue}33`, textTransform: "uppercase", letterSpacing: 0.5 }}>
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: 5, cursor: "pointer", padding: "4px 10px", borderRadius: 4, background: alt.accepted ? "rgba(76,175,125,0.15)" : "transparent", border: `1px solid ${alt.accepted ? C.green : C.border}` }}>
                      <input type="checkbox" checked={alt.accepted} onChange={e => updateAlternate(alt.id, "accepted", e.target.checked)} style={{ accentColor: C.green }} />
                      <span style={{ fontSize: 10, fontWeight: 600, color: alt.accepted ? C.green : C.textDim }}>Accepted</span>
                    </label>
                    <button className="icon-btn" onClick={() => removeAlternate(alt.id)} style={{ width: 24, height: 24, border: "none", background: "transparent", color: C.red, borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                  ...
                  <div style={{ padding: "8px 14px", borderBottom: `1px solid ${C.border}` }}>
                    <input value={alt.description} onChange={e => updateAlternate(alt.id, "description", e.target.value)} placeholder="Description of this alternate (shown on proposals)..." style={inp(C, { width: "100%", fontSize: 11, padding: "5px 10px" })} />
                  </div>

                  {/* Line Items */}
                  <div style={{ padding: "8px 14px" }}>
                    {alt.items.length > 0 && (
                      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1.2fr .5fr .5fr .7fr .7fr .7fr .7fr .8fr auto", gap: 6, marginBottom: 6, fontSize: 8, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>
                        <span>Description</span><span>Linked Item</span><span>Qty</span><span>Unit</span><span style={{ textAlign: "right" }}>Material</span><span style={{ textAlign: "right" }}>Labor</span><span style={{ textAlign: "right" }}>Equip</span><span style={{ textAlign: "right" }}>Sub</span><span style={{ textAlign: "right" }}>Total</span><span></span>
                      </div>
                    )}
                    {alt.items.map(ai => {
                      ...
                      return (
                        <div key={ai.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 1.2fr .5fr .5fr .7fr .7fr .7fr .7fr .8fr auto", gap: 6, marginBottom: 4, alignItems: "center" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            {ai.linkedItemId && alt.accepted && (
                              <span style={{ fontSize: 7, fontWeight: 700, color: C.orange || "#F59E0B", background: "rgba(245,158,11,0.12)", padding: "1px 4px", borderRadius: 3, flexShrink: 0 }}>REPLACES</span>
                            )}
                            <input value={ai.description} onChange={e => updateAlternateItem(alt.id, ai.id, "description", e.target.value)} placeholder="Line item..." style={inp(C, { fontSize: 11, padding: "4px 8px", flex: 1 })} />
                          </div>
                          <select
                            value={ai.linkedItemId || ""}
                            onChange={e => linkAlternateItem(alt.id, ai.id, e.target.value || null)}
                            style={inp(C, { fontSize: 10, padding: "4px 4px", color: ai.linkedItemId ? C.text : C.textDim })}
                          >
                            <option value="">— None —</option>
                            {items.map(it => (
                              <option key={it.id} value={it.id}>
                                {it.code ? `${it.code} ` : ""}{it.description || "Unnamed item"}
                              </option>
                            ))}
                          </select>
                          <input type="number" value={ai.quantity} onChange={e => updateAlternateItem(alt.id, ai.id, "quantity", e.target.value)} style={nInp(C, { fontSize: 11, padding: "4px 6px", textAlign: "center" })} />
                          <select value={ai.unit} onChange={e => updateAlternateItem(alt.id, ai.id, "unit", e.target.value)} style={inp(C, { fontSize: 10, padding: "4px 4px" })}>
                            {["LS", "SF", "LF", "EA", "SY", "CY", "CF", "GAL", "TON", "HR", "DAY", "MO", "VLF"].map(u => <option key={u} value={u}>{u}</option>)}
                          </select>
                          <input type="number" value={ai.material} onChange={e => updateAlternateItem(alt.id, ai.id, "material", e.target.value)} placeholder="0" style={nInp(C, { fontSize: 11, padding: "4px 6px", textAlign: "right" })} />
                    ...
                    <button className="ghost-btn" onClick={() => addAlternateItem(alt.id)} style={bt(C, { background: "transparent", border: `1px dashed ${C.border}`, color: C.accent, padding: "5px 12px", fontSize: 10, marginTop: 4 })}>
        ...
        {/* Summary */}
        {alternates.length > 0 && (
          <div style={{ marginTop: 16, padding: "12px 16px", background: C.bg1, borderRadius: 6, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>Alternates Summary</div>
            {totals.altTotals.map(alt => (
              <div key={alt.id} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: `1px solid ${C.bg2}`, alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, color: alt.type === "deduct" ? C.green : C.blue }}>{alt.type === "deduct" ? "DEDUCT" : "ADD"}</span>
                  <span style={{ fontSize: 12, color: C.text }}>{alt.name}</span>
                  {alt.accepted && <span style={{ fontSize: 8, fontWeight: 700, color: C.green, background: "rgba(76,175,125,0.12)", padding: "1px 6px", borderRadius: 3 }}>ACCEPTED</span>}
                  {alt.items && alt.items.some(it => it.linkedItemId) && alt.accepted && (
                    <span style={{ fontSize: 8, fontWeight: 700, color: C.orange || "#F59E0B", background: "rgba(245,158,11,0.10)", padding: "1px 6px", borderRadius: 3 }}>SWAPS ITEMS</span>
                  )}
                </div>
