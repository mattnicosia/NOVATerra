...
export function useDashboardData() {
  ...
  const handleOpenEstimate = useCallback((id) => {
  ...
  const benchmarks = useMemo(() => {
    ...
    const costPerSF = withSF.length > 0
      ? withSF.reduce((s, e) => s + nn(e.grandTotal) / nn(e.projectSF), 0) / withSF.length
      : 0;
  ...
  const handleDeleteEstimate = useCallback(async (id) => {
