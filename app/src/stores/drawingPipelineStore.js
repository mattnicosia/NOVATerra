...
export const useDrawingPipelineStore = create((set, get) => ({
  ...
  toggleTakeoffVisibility: id => set(s => {
    const next = new Set(s.hiddenTakeoffIds);
    if (next.has(id)) next.delete(id);
    else next.add(id);
    return { hiddenTakeoffIds: next };
  }),
  showAllTakeoffs: () => set({ hiddenTakeoffIds: new Set() }),
  ...
  addTakeoff: (group, desc, unit, code, bidContext) => {
    ...
    // Note: takeoff creation is NOT pushed to undo stack.
    // Users delete takeoffs explicitly — accidental undo-delete loses work.
    return newId;
  },
