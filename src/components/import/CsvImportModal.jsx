  ...
  const handleFile = useCallback((file) => {
    ...
    reader.onload = async (e) => {
      ...
      if (apiKey) {
        setAiLoading(true);
        try {
          const suggested = await suggestColumnMappings(apiKey, parsed.headers, parsed.rows.slice(0, 5));
      ...
      {step === "mapping" && (
        <div>
          ...
          {!apiKey && !aiLoading && (
            <div style={{
              padding: `${T.space[2]}px ${T.space[3]}px`,
              background: C.bg, borderRadius: T.radius.sm, marginBottom: T.space[3],
              border: `1px solid ${C.border}`,
