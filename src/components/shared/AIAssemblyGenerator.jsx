...
export default function AIAssemblyGenerator({ onClose }) {
  ...
  const generate = async () => {
    ...
    setLoading(true); setError(""); setResult(null); setStream("");
    ...
    try {
      ...
      if (err.name === "AbortError") { setStream(""); setLoading(false); return; }
  ...
  const addToDatabase = () => {
    ...
    addAssembly({
      ...
      elements: result.elements.map(el => ({
        code: el.code, desc: titleCase(el.desc), unit: el.unit,
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: 720, maxHeight: "85vh", background: C.bg,
        borderRadius: T.radius.lg, border: `1px solid ${C.border}`,
        boxShadow: T.shadow.lg, display: "flex", flexDirection: "column",
        overflow: "hidden",
      }}>
        {/* Header */}
        <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 10, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 6, display: "block" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <textarea
                ...
                style={{
                  ...inp(C),
                  flex: 1, resize: "vertical", fontSize: 13, padding: "10px 14px",
              ...
              {loading ? (
                <button onClick={stopGeneration} style={{
                  ...bt(C), background: C.red, color: "#fff", padding: "10px 18px",
                  fontSize: 12, fontWeight: 700, borderRadius: T.radius.sm, alignSelf: "flex-end",
                ...
                <button onClick={generate} disabled={!prompt.trim()} style={{
                  ...bt(C), background: prompt.trim() ? `linear-gradient(135deg, ${C.accent}, ${C.accentAlt || C.purple})` : C.bg2,
                  color: prompt.trim() ? "#fff" : C.textDim, padding: "10px 18px",
                  fontSize: 12, fontWeight: 700, borderRadius: T.radius.sm, alignSelf: "flex-end",
          ...
          {!result && !loading && !stream && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: C.textDim, marginBottom: 8 }}>
                Try an example:
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {EXAMPLE_PROMPTS.map((ex, i) => (
                  <button key={i} onClick={() => setPrompt(ex)} style={{
                    ...bt(C), background: C.bg2, color: C.textMuted, border: `1px solid ${C.border}`,
          ...
          {error && (
            <div style={{
              padding: "10px 14px", background: `${C.red}15`, border: `1px solid ${C.red}30`,
              borderRadius: 6, marginBottom: 14, fontSize: 12, color: C.red, fontWeight: 500,
            }}>
              {error}
            </div>
          )}
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  <input
                    ...
                    style={{
                      ...inp(C), flex: 1, fontSize: 14, fontWeight: 700, padding: "4px 8px",
                ...
                <input
                  ...
                  style={{
                    ...inp(C), width: "100%", fontSize: 11, color: C.textMuted, padding: "3px 8px",
                    background: "transparent", border: `1px solid transparent`,
                  }}
                  onFocus={(e) => e.target.style.borderColor = C.accent}
                  onBlur={(e) => e.target.style.borderColor = "transparent"}
              ...
              <div style={{ padding: "0 0 8px" }}>
                ...
                <div style={{
                  ...
                  <span>Code</span><span>Description</span><span>Unit</span>
                ...
                {result.elements.map((el, idx) => {
                  ...
                  return (
                    <div key={idx} style={{
                      display: "grid",
                      gridTemplateColumns: "70px 2fr 44px 72px 72px 72px 50px 80px 28px",
                      gap: 4, padding: "5px 12px", borderBottom: `1px solid ${C.bg}`,
                      ...
                      {isEditing ? (<>
                        ...
                        <input type="number" value={el.m} onChange={e => updateElement(idx, "m", parseFloat(e.target.value) || 0)}
                          style={nInp(C, { fontSize: 10, padding: "2px 4px", color: C.green })} />
                        <input type="number" value={el.l} onChange={e => updateElement(idx, "l", parseFloat(e.target.value) || 0)}
                          style={nInp(C, { fontSize: 10, padding: "2px 4px", color: C.blue })} />
                        <input type="number" value={el.e} onChange={e => updateElement(idx, "e", parseFloat(e.target.value) || 0)}
                        ...
                        <div style={{ display: "flex", gap: 1 }}>
                          ...
                          <button onClick={() => removeElement(idx)} title="Remove" style={{
                            width: 18, height: 18, border: "none", background: "transparent",
                            borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center",
              ...
              <div style={{
                ...
                background: C.bg2, display: "flex", justifyContent: "space-between", alignItems: "center",
              }}>
                <span style={{ fontSize: 11, color: C.textMuted }}>
                  {result.elements.length} elements &bull;{" "}
                  <span style={{ color: C.green }}>M: {fmt2(result.elements.reduce((s, el) => s + nn(el.m) * nn(el.factor || 1), 0))}</span>{" "}
                  <span style={{ color: C.blue }}>L: {fmt2(result.elements.reduce((s, el) => s + nn(el.l) * nn(el.factor || 1), 0))}</span>{" "}
                ...
                <span style={{
                  ...
                  {fmt2(totalPer)}<span style={{ fontSize: 10, color: C.textDim, fontWeight: 500 }}>/unit</span>
                </span>
              </div>
            </div>
          )}
        ...
        {result && (
          <div style={{
            ...
            <button onClick={() => { setResult(null); setStream(""); }} style={{
              ...bt(C), background: "transparent", border: `1px solid ${C.border}`,
              color: C.textMuted, padding: "8px 16px", fontSize: 11, borderRadius: T.radius.sm,
              cursor: "pointer",
            }}>
              Regenerate
            </button>
            <button onClick={addToDatabase} style={{
              ...bt(C), background: `linear-gradient(135deg, ${C.accent}, ${C.accentAlt || C.purple})`,
              color: "#fff", padding: "8px 20px", fontSize: 12, fontWeight: 700,
