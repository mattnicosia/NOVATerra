...
export default function ScopeOfWork({ data }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
            ...
            {isNarrative ? (
              ...
              groupItems.map(item => (
                <div key={item.id} style={{
                  ...
                  {scopeShowQuantities && (
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: "#888", whiteSpace: "nowrap" }}>
      ...
      {!apiKey && (
        <div className="no-print" style={{ fontSize: 9, color: "#aaa", marginTop: 4, fontStyle: "italic" }}>
          Add an API key in Settings to enable AI scope summaries.
        </div>
      )}
    </div>
  );
