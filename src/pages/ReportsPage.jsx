...
export default function ReportsPage() {
  ...
  const [sovMode, setSovMode] = useState("below");
  const [sovSort, setSovSort] = useState("trade");
    ...
    <div style={{ padding: T.space[7], minHeight: "100%", animation: "fadeIn 0.15s ease-out" }}>
      <div style={{ maxWidth: 1000 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12, justifyContent: "flex-end" }}>
          <button className="accent-btn" onClick={() => window.print()} style={bt(C, { background: C.blue, color: "#fff", padding: "7px 14px", fontSize: 11 })}>
          ...
          <Sec title="Cost Summary">
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", padding: "14px 0 6px", borderTop: `2px solid ${C.accent}`, marginTop: 4 }}>
                ...
                <span style={{ fontSize: 20, fontWeight: 700, color: C.accent, fontFamily: "'DM Mono',monospace" }}>{fmt(totals.grand)}</span>
              </div>
              {totals.coT !== 0 && (<>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 4px", borderTop: `1px solid ${C.border}` }}>
                  ...
                  <span style={{ fontSize: 18, fontWeight: 700, color: C.purple, fontFamily: "'DM Mono',monospace" }}>{fmt(totals.revised)}</span>
                </div>
              </>)}
            </div>
          </Sec>
            ...
            {tradeBreakdown.map(trade => {
              ...
              return (
                <div key={trade.label} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: `1px solid ${C.bg}` }}>
                  ...
                  <div style={{ width: 50, textAlign: "right", fontFamily: "'DM Mono',monospace", fontSize: 11, color: C.textMuted }}>{pct(p)}</div>
                  <div style={{ width: 100, textAlign: "right", fontFamily: "'DM Mono',monospace", fontSize: 12, fontWeight: 600, color: C.text }}>{fmt(trade.total)}</div>
                </div>
              );
            })}
          </Sec>
          {nn(project.projectSF) > 0 && (
            <div style={{ padding: "10px 14px", background: C.bg1, borderRadius: T.radius.sm, border: `1px solid ${C.border}`, marginTop: 8, boxShadow: T.shadow.sm }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, color: C.accent }}>{fmt2(totals.grand / nn(project.projectSF))}/SF</span>
              </div>
            </div>
          )}
        </>)}
        ...
        {/* SCHEDULE OF VALUES */}
        {reportType === "sov" && (() => {
          // Modern font stack for the SOV document
          ...
          return (
            <div style={{ display: "flex", gap: 0 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div id="proposal-print" style={{ background: "#fff", color: "#1a1a2e", padding: "40px 48px", borderRadius: T.radius.lg, border: `1px solid ${C.border}`, fontFamily: "'DM Sans',sans-serif", lineHeight: 1.6, boxShadow: T.shadow.lg }}>
                  {sectionOrder
        ...
        {/* BID FORM */}
        {reportType === "bidForm" && (
          <div style={{ background: "#fff", color: "#1a1a2e", padding: "40px 48px", borderRadius: 8, border: `1px solid ${C.border}`, fontFamily: "'DM Sans',sans-serif" }}>
            ...
            <div style={{ padding: "14px 20px", background: "#f0f0f0", borderRadius: 4, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "'DM Mono',monospace" }}>{fmt(totals.grand)}</div>
            </div>
            ...
            {alternates.map((alt, i) => {
              ...
              return (
                <div key={alt.id} style={{ padding: "10px 20px", background: "#f8f8f8", borderRadius: 4, marginBottom: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "'DM Mono',monospace" }}>{alt.type === "deduct" ? "\u2212" : "+"}{fmt(t)}</div>
                </div>
              );
            })}
                  ...
                  {divItems.map(item => {
                    ...
                    return (
                      <div key={item.id} style={{ display: "grid", gridTemplateColumns: "60px 2fr .5fr .4fr .7fr .7fr .7fr .7fr .8fr", gap: 6, padding: "4px 0", borderBottom: `1px solid ${C.bg2}`, alignItems: "center", fontSize: 11 }}>
                        <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 9, color: C.purple }}>{item.code}</span>
                        <span style={{ color: C.text }}>{item.description}</span>
                        <span style={{ textAlign: "right", fontFamily: "'DM Mono',monospace" }}>{q}</span>
                        <span style={{ color: C.textDim, fontSize: 10 }}>{item.unit}</span>
                        <span style={{ textAlign: "right", fontFamily: "'DM Mono',monospace" }}>{nn(item.material) ? fmt2(nn(item.material)) : ""}</span>
                        <span style={{ textAlign: "right", fontFamily: "'DM Mono',monospace" }}>{nn(item.labor) ? fmt2(nn(item.labor)) : ""}</span>
                        <span style={{ textAlign: "right", fontFamily: "'DM Mono',monospace" }}>{nn(item.equipment) ? fmt2(nn(item.equipment)) : ""}</span>
                        <span style={{ textAlign: "right", fontFamily: "'DM Mono',monospace" }}>{nn(item.subcontractor) ? fmt2(nn(item.subcontractor)) : ""}</span>
                        <span style={{ textAlign: "right", fontWeight: 600, fontFamily: "'DM Mono',monospace" }}>{fmt(t)}</span>
                      </div>
                    );
                  })}
                </Sec>
                ...
                return (
                  <Sec key={alt.id} title={`Alternate #${i + 1}: ${alt.name} (${alt.type === "deduct" ? "DEDUCT" : "ADD"})`}>
                    ...
                    {alt.items.map(ai => {
                      ...
                      return (
                        <div key={ai.id} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: `1px solid ${C.bg2}`, fontSize: 11 }}>
                          ...
                          <span style={{ fontFamily: "'DM Mono',monospace", fontWeight: 600 }}>{fmt(it)}</span>
                        </div>
                      );
                    })}
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0 4px", borderTop: `1px solid ${C.border}`, marginTop: 4 }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono',monospace", fontWeight: 700, color: alt.type === "deduct" ? C.green : C.blue }}>{alt.type === "deduct" ? "\u2212" : "+"}{fmt(t)}</span>
                    </div>
                  </Sec>
                );
              })
