...
import { nn, fmt, fmt2, pct, uid } from '@/utils/format';
...
function LocalInput({ value, onCommit, ...props }) {
  ...
  return <input ref={ref} {...props} value={local} onChange={e => setLocal(e.target.value)} onBlur={() => onCommit(local)} />;
}
...
export default function ScheduleOfValuesPage() {
  ...
  const estimateTotals = useMemo(() => {
    ...
    return { material, labor, equipment, sub, direct, markupAmounts, subtotal, customAmounts, customTotal, afterCustom, bd, tx, grand };
  ...
  const sovData = useMemo(() => {
    ...
    items.forEach(item => {
      ...
      if (sovGroupBy === "trade") {
        ...
        label = customLbl ? `${key} \u2014 ${customLbl}` : `${key} \u2014 ${activeCodes[dc]?.subs?.[key] || activeCodes[dc]?.name || key}`;
      } else {
        key = item.division || "Unassigned";
        label = key;
      }
      if (!groups[key]) groups[key] = { label, material: 0, labor: 0, equipment: 0, sub: 0, items: [], subKeys: new Set(), sortVal: sortVal ?? 0 };
    ...
    const grandDirectPreCalc = Object.values(groups).reduce((s, g) => s + (g.material * catMkup.material + g.labor * catMkup.labor + g.equipment * catMkup.equipment + g.sub * catMkup.sub), 0);
    const sortedGroups = Object.entries(groups).sort(([a, ag], [b, bg]) =>
      sovGroupBy === "trade" ? ag.sortVal - bg.sortVal : a.localeCompare(b)
    );
    let grandDirect = 0, grandMarkup = 0, grandTotal = 0;
    ...
    return { rowData, grandDirect, grandMarkup, grandTotal, grandMat, grandLab, grandEqp, grandSub, hasCatMkup, catMkup };
  }, [items, markup, markupOrder, customMarkups, sovGroupBy, sovCatMarkup, activeCodes, bidSelections, subKeyLabels]);

  // Draggable block wrapper
  const DBlock = ({ id, title, children }) => (
    <div draggable onDragStart={() => { sovDragItem.current = id; }} onDragEnter={() => { sovDragOver.current = id; }} onDragEnd={sovDragSort} onDragOver={e => e.preventDefault()}
      style={{ marginBottom: 14, background: C.bg1, borderRadius: 8, border: `1px solid ${C.border}`, overflow: "hidden", transition: "box-shadow 0.15s" }}>
      <div style={{ padding: "6px 14px", display: "flex", alignItems: "center", gap: 8, cursor: "grab", borderBottom: `1px solid ${C.border}`, background: C.bg2, userSelect: "none" }}
        onMouseDown={e => e.currentTarget.style.cursor = "grabbing"} onMouseUp={e => e.currentTarget.style.cursor = "grab"}>
        <svg width="10" height="14" viewBox="0 0 10 14" fill={C.textDim}><circle cx="3" cy="2" r="1.2" /><circle cx="7" cy="2" r="1.2" /><circle cx="3" cy="7" r="1.2" /><circle cx="7" cy="7" r="1.2" /><circle cx="3" cy="12" r="1.2" /><circle cx="7" cy="12" r="1.2" /></svg>
        <span style={{ fontSize: 10, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, flex: 1 }}>{title}</span>
      </div>
  ...
  const blocks = {
    "sov-grid": (
      <DBlock key="sov-grid" id="sov-grid" title="Schedule of Values Grid">
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <div style={{ display: "flex", background: C.bg2, borderRadius: 4, overflow: "hidden" }}>
              {["trade", "subdivision", "division"].map(g => (
                <button key={g} onClick={() => setSovGroupBy(g)} style={{ padding: "5px 12px", fontSize: 10, fontWeight: 600, border: "none", cursor: "pointer", background: sovGroupBy === g ? C.accent : "transparent", color: sovGroupBy === g ? "#fff" : C.textMuted }}>{g === "trade" ? "By Trade Bundle" : g === "subdivision" ? "By Subdivision" : "By Division"}</button>
              ))}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", background: C.accentBg, borderRadius: 6, border: `1px solid ${C.accent}30` }}>
                <span style={{ fontSize: 8, fontWeight: 700, color: C.accent }}>All</span>
                <input type="number" placeholder="0"
                  onChange={e => { const v = e.target.value; setSovCatMarkup({ material: v, labor: v, equipment: v, sub: v }); }}
                  value={sovCatMarkup.material === sovCatMarkup.labor && sovCatMarkup.labor === sovCatMarkup.equipment && sovCatMarkup.equipment === sovCatMarkup.sub ? sovCatMarkup.material : ""}
                  style={nInp(C, { width: 42, padding: "2px 3px", fontSize: 10, fontWeight: 700, textAlign: "center", border: `1px solid ${C.accent}30` })} />
                <span style={{ fontSize: 8, color: C.textDim }}>%</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", background: C.bg, borderRadius: 6, border: `1px solid ${C.border}` }}>
                <span style={{ fontSize: 8, fontWeight: 600, color: C.textDim }}>Adjust</span>
                {[{ k: "material", label: "M", color: C.green }, { k: "labor", label: "L", color: C.blue }, { k: "equipment", label: "E", color: C.orange }, { k: "sub", label: "S", color: C.purple }].map(c => (
                  <div key={c.k} style={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <span style={{ fontSize: 8, fontWeight: 700, color: c.color }}>{c.label}</span>
                    <input type="number" value={sovCatMarkup[c.k]} onChange={e => setSovCatMarkup(p => ({ ...p, [c.k]: e.target.value }))} placeholder="0" style={nInp(C, { width: 36, padding: "2px 3px", fontSize: 10, fontWeight: 700, textAlign: "center" })} />
                    <span style={{ fontSize: 8, color: C.textDim }}>%</span>
                  </div>
                ))}
              </div>
        ...
        {/* Column headers */}
        <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "7px 12px", fontSize: 8, fontWeight: 700, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, borderBottom: `2px solid ${C.border}`, background: C.bg2, borderRadius: "4px 4px 0 0" }}>
          <div style={{ flex: 3, minWidth: 180 }}>{sovGroupBy === "trade" ? "Trade Bundle" : sovGroupBy === "subdivision" ? "Subdivision" : "Division"}</div>
          <div style={{ width: 42, textAlign: "center" }}>Items</div>
          <div style={{ width: 80, textAlign: "right" }}>Material{nn(sovCatMarkup.material) ? ` (${nn(sovCatMarkup.material) > 0 ? "+" : ""}${sovCatMarkup.material}%)` : ""}</div>
          <div style={{ width: 80, textAlign: "right" }}>Labor{nn(sovCatMarkup.labor) ? ` (${nn(sovCatMarkup.labor) > 0 ? "+" : ""}${sovCatMarkup.labor}%)` : ""}</div>
          <div style={{ width: 75, textAlign: "right" }}>Equip{nn(sovCatMarkup.equipment) ? ` (${nn(sovCatMarkup.equipment) > 0 ? "+" : ""}${sovCatMarkup.equipment}%)` : ""}</div>
          <div style={{ width: 80, textAlign: "right" }}>Sub{nn(sovCatMarkup.sub) ? ` (${nn(sovCatMarkup.sub) > 0 ? "+" : ""}${sovCatMarkup.sub}%)` : ""}</div>
        ...
        {sovData.rowData.map(r => (
          <div key={r.key} className="row" style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderBottom: `1px solid ${C.bg2}` }}>
            <div style={{ flex: 3, minWidth: 180 }}><div style={{ fontSize: 11, fontWeight: 600, color: C.text }}>{r.g.label}</div></div>
            <div style={{ width: 42, textAlign: "center", fontSize: 10, color: C.textDim }}>{r.g.items.length}</div>
            <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, color: nn(sovCatMarkup.material) ? C.green : C.textMuted }}>{fmt(r.adjMat)}</div>
            <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, color: nn(sovCatMarkup.labor) ? C.blue : C.textMuted }}>{fmt(r.adjLab)}</div>
            <div style={{ width: 75, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, color: nn(sovCatMarkup.equipment) ? C.orange : C.textMuted }}>{fmt(r.adjEqp)}</div>
            <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, color: nn(sovCatMarkup.sub) ? C.purple : C.textMuted }}>{fmt(r.adjSub)}</div>
            <div style={{ width: 95, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 11, fontWeight: 600, color: C.text }}>{fmt(r.direct)}</div>
            <div style={{ width: 95, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, color: C.textMuted }}>{fmt(r.afterCustom)}</div>
            <div style={{ width: 100, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 12, fontWeight: 700, color: C.accent }}>{fmt(r.withBondTax)}</div>
            <div style={{ width: 72, textAlign: "center" }}><span style={{ fontSize: 8, fontWeight: 700, padding: "2px 6px", borderRadius: 3, background: `${r.src.color}18`, color: r.src.color, border: `1px solid ${r.src.color}30` }}>{r.src.label}</span></div>
          </div>
        ))}

        {/* Totals row */}
        <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "10px 12px", borderTop: `2px solid ${C.accent}`, background: C.bg2, borderRadius: "0 0 4px 4px" }}>
          <div style={{ flex: 3, minWidth: 180, fontSize: 12, fontWeight: 700, color: C.accent }}>TOTALS</div>
          <div style={{ width: 42, textAlign: "center", fontSize: 10, fontWeight: 600, color: C.text }}>{items.length}</div>
          <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, fontWeight: 700, color: C.green }}>{fmt(sovData.grandMat)}</div>
          <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, fontWeight: 700, color: C.blue }}>{fmt(sovData.grandLab)}</div>
          <div style={{ width: 75, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, fontWeight: 700, color: C.orange }}>{fmt(sovData.grandEqp)}</div>
          <div style={{ width: 80, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 10, fontWeight: 700, color: C.purple }}>{fmt(sovData.grandSub)}</div>
          <div style={{ width: 95, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 12, fontWeight: 700, color: C.text }}>{fmt(sovData.grandDirect)}</div>
          <div style={{ width: 95, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 12, fontWeight: 600, color: C.textMuted }}>{fmt(sovData.grandMarkup)}</div>
          <div style={{ width: 100, textAlign: "right", fontFeatureSettings: "'tnum'", fontSize: 14, fontWeight: 700, color: C.accent }}>{fmt(sovData.grandTotal)}</div>
          <div style={{ width: 72 }} />
        </div>

        {sovData.hasCatMkup && (
          <div style={{ marginTop: 8, padding: "6px 10px", background: C.accentBg, borderRadius: 4, fontSize: 10, color: C.textMuted }}>
            <strong style={{ color: C.accent }}>Category Adjustments:</strong>{" "}
            {nn(sovCatMarkup.material) !== 0 && <span>Material <strong style={{ color: C.green }}>{nn(sovCatMarkup.material) > 0 ? "+" : ""}{sovCatMarkup.material}%</strong>{" "}</span>}
            {nn(sovCatMarkup.labor) !== 0 && <span>Labor <strong style={{ color: C.blue }}>{nn(sovCatMarkup.labor) > 0 ? "+" : ""}{sovCatMarkup.labor}%</strong>{" "}</span>}
            {nn(sovCatMarkup.equipment) !== 0 && <span>Equipment <strong style={{ color: C.orange }}>{nn(sovCatMarkup.equipment) > 0 ? "+" : ""}{sovCatMarkup.equipment}%</strong>{" "}</span>}
            {nn(sovCatMarkup.sub) !== 0 && <span>Sub <strong style={{ color: C.purple }}>{nn(sovCatMarkup.sub) > 0 ? "+" : ""}{sovCatMarkup.sub}%</strong>{" "}</span>}
          </div>
        )}
      </DBlock>
    ),
      ...
      <DBlock key="comparison" id="comparison" title="Estimate vs SOV Comparison">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", gap: 20, fontSize: 12 }}>
            <span style={{ color: C.textMuted }}>Estimate Grand Total: <strong style={{ color: C.text, fontFeatureSettings: "'tnum'" }}>{fmt(estimateTotals.grand)}</strong></span>
            <span style={{ color: C.textMuted }}>SOV Total: <strong style={{ color: C.accent, fontFeatureSettings: "'tnum'" }}>{fmt(sovData.grandTotal)}</strong></span>
            {(sovData.hasCatMkup || sovData.grandTotal !== estimateTotals.grand) && (
              <span style={{ color: sovData.grandTotal >= estimateTotals.grand ? C.green : C.red }}>
                {"\u0394"}: <strong style={{ fontFeatureSettings: "'tnum'" }}>{sovData.grandTotal >= estimateTotals.grand ? "+" : ""}{fmt(sovData.grandTotal - estimateTotals.grand)}</strong>
              </span>
            )}
          </div>
      ...
      <DBlock key="markup" id="markup" title="Markup & Adjustments">
        ...
        {customMarkups.length > 0 && (
          <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10, marginBottom: 8 }}>
            <div style={{ fontSize: 9, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 6 }}>Additional Markups</div>
            {customMarkups.map(cm => {
              ...
              return (
                <div key={cm.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <LocalInput value={cm.label} onCommit={v => updateCustomMarkup(cm.id, "label", v)}
                    placeholder="Label (e.g. Design Fee, Supervision...)" style={inp(C, { flex: 1, maxWidth: 260, padding: "5px 8px", fontSize: 11 })} />
                  <LocalInput type="number" value={cm.value} onCommit={v => updateCustomMarkup(cm.id, "value", v)}
                    placeholder="0" style={nInp(C, { width: 72, padding: "5px 8px", fontSize: 12, fontWeight: 600 })} />
                  <select value={cm.type} onChange={e => updateCustomMarkup(cm.id, "type", e.target.value)}
                    style={inp(C, { width: 60, padding: "5px 4px", fontSize: 10 })}>
                    <option value="pct">%</option>
                    <option value="flat">$</option>
                  </select>
                  {amt && <span style={{ fontSize: 10, color: C.accent, fontWeight: 600, fontFeatureSettings: "'tnum'", minWidth: 80, textAlign: "right" }}>{fmt(amt.amount)}</span>}
                  <button className="icon-btn" onClick={() => setCustomMarkups(customMarkups.filter(c => c.id !== cm.id))}
                    style={{ width: 20, height: 20, border: "none", background: "transparent", color: C.red, borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", opacity: 0.6 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `1px solid ${C.border}`, paddingTop: 8 }}>
          <button className="ghost-btn" onClick={() => setCustomMarkups([...customMarkups, { id: uid(), label: "", value: "", type: "pct" }])}
            style={bt(C, { background: "transparent", border: `1px solid ${C.border}`, color: C.accent, padding: "5px 12px", fontSize: 10 })}>
          ...
          <div style={{ display: "flex", gap: 16, fontSize: 11, fontFeatureSettings: "'tnum'" }}>
            <span style={{ color: C.textDim }}>Direct: <strong style={{ color: C.text }}>{fmt(estimateTotals.direct)}</strong></span>
            <span style={{ color: C.textDim }}>Subtotal: <strong style={{ color: C.text }}>{fmt(estimateTotals.subtotal)}</strong></span>
            {estimateTotals.customTotal !== 0 && <span style={{ color: C.textDim }}>+ Custom: <strong style={{ color: C.accent }}>{fmt(estimateTotals.customTotal)}</strong></span>}
