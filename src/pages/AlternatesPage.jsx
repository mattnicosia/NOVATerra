...
export default function AlternatesPage() {
  ...
  const totals = useMemo(() => {
    ...
    const altTotals = alternates.map(alt => {
      const t = getAltTotal(alt);
      return { ...alt, total: alt.type === "deduct" ? -t : t };
    });
  ...
  }, [items, markup, markupOrder, customMarkups, alternates, getAltTotal]);
  ...
  const runVE = async () => {
    ...
    try {
      ...
      const text = await callAnthropic({ apiKey, max_tokens: 1500, messages: [{ role: "user", content: prompt }] });
        ...
        {alternates.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {alternates.map((alt, altIdx) => {
              const altTotal = getAltTotal(alt);
              return (
                <div key={alt.id} style={{ border: `1px solid ${alt.accepted ? "rgba(76,175,125,0.4)" : C.border}`, borderLeft: `3px solid ${alt.type === "deduct" ? C.green : C.blue}`, borderRadius: T.radius.md, overflow: "hidden", background: alt.accepted ? "rgba(76,175,125,0.03)" : "transparent" }}>
                  ...
                  <div style={{ padding: "10px 14px", background: C.bg1, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, color: alt.type === "deduct" ? C.green : C.blue, minWidth: 100, textAlign: "right" }}>
                      {alt.type === "deduct" ? "\u2212" : "+"}{fmt(altTotal)}
                    </div>
                  ...
                  <div style={{ padding: "8px 14px" }}>
                    {alt.items.length > 0 && (
                      <div style={{ display: "grid", gridTemplateColumns: "2fr .5fr .5fr .7fr .7fr .7fr .7fr .8fr auto", gap: 6, marginBottom: 6, fontSize: 8, fontWeight: 600, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5 }}>
                        <span>Description</span><span>Qty</span><span>Unit</span><span style={{ textAlign: "right" }}>Material</span><span style={{ textAlign: "right" }}>Labor</span><span style={{ textAlign: "right" }}>Equip</span><span style={{ textAlign: "right" }}>Sub</span><span style={{ textAlign: "right" }}>Total</span><span></span>
                      </div>
                    )}
                    {alt.items.map(ai => {
                      ...
                      return (
                        <div key={ai.id} style={{ display: "grid", gridTemplateColumns: "2fr .5fr .5fr .7fr .7fr .7fr .7fr .8fr auto", gap: 6, marginBottom: 4, alignItems: "center" }}>
                          <input value={ai.description} onChange={e => updateAlternateItem(alt.id, ai.id, "description", e.target.value)} placeholder="Line item..." style={inp(C, { fontSize: 11, padding: "4px 8px" })} />
